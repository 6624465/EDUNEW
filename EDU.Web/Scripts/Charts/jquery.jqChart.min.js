...
(function (a) {
    ...
    a.fn.jqChart.defaults = {
        title: {
            margin: 8, font: "22px sans-serif"
        }
        , tooltips: {
            disabled: false, type: "normal", borderColor: "auto", snapArea: 25, highlighting: true, highlightingFillStyle: "rgba(204,  204,  204,  0.5)", highlightingStrokeStyle: "rgba(204,  204,  204,  0.5)"
        }
        , crosshairs: {
            enabled: false, snapToDataPoints: true, hLine: {
                visible: true, strokeStyle: "red"
            }
            , vLine: {
                visible: true, strokeStyle: "red"
            }
        }, globalAlpha: 1, mouseInteractionMode: "panning", mouseWheelInteractionMode: "zooming", selectionRect: {
            fillStyle: "rgba(125, 125, 125, 0.2)", strokeStyle: "gray", lineWidth: 0;
        }
        , shadows: {
            enabled: false, shadowColor: "#cccccc", shadowBlur: 8, shadowOffsetX: 2, shadowOffsetY: 2;
        }
        , watermark: {
            hAlign: "right", vAlign: "bottom"
        }
        , noDataMessage: {
            text: "No data available", font: "20px sans-serif"
        }
        , exportConfig: {
    ...
    a.jqChartSprintf = function () {
        ...
        return h.replace(a.jqChartSprintf.regex, function (t, s, q, a, w, h, m) {
            ...
            switch (m) {
                case "s": return g(String(i), j, a, h, k);
...
|yy(?: yy) ?|([HhMsTt]) \1 ?|[LloSZ] | "[^"]*"|'[^']*'/g, d=/\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d {
    4;
}
) ?)\b / g, c = /[^-+\dA-Z]/g, b = function (a, b) {
    a = String(a);
    b = b || 2;
...
return function (f, h, l) {
    ...
    var j = l ? "getUTC" : "get", o = f[j + "Date"](), r = f[j + "Day"](), n = f[j + "Month"](), u = f[j + "FullYear"](), i = f[j + "Hours"](), s = f[j + "Minutes"](), t = f[j + "Seconds"](), m = f[j + "Milliseconds"](), p = l ? 0 : f.getTimezoneOffset(), q = {
        d: o, dd: b(o), ddd: k.dayNames[r], dddd: k.dayNames[r + 7], m: n + 1, mm: b(n + 1), mmm: k.monthNames[n], mmmm: k.monthNames[n + 12], yy: String(u).slice(2), yyyy: u, h: i % 12 || 12, hh: b(i % 12 || 12), H: i, HH: b(i), M: s, MM: b(s), s: t, ss: b(t), l: b(m, 3), L: b(m > 99 ? Math.round(m / 10) : m), t: i < 12 ? g[0].charAt(0) || "" : g[1].charAt(0) || "", tt: i < 12 ? g[0] : g[1], T: i < 12 ? g[2].charAt(0) || "" : g[3].charAt(0) || "", TT: i < 12 ? g[2] : g[3], Z: l ? "UTC" : (String(f).match(d) || [""]).pop().replace(c, ""), o: (p > 0 ? "-" : "+") + b(Math.floor(Math.abs(p) / 60) * 100 + Math.abs(p) % 60, 4), S: k.s(o);
    }
        ;
...
a.jqChartDateFormat = {
    masks: {
        shortDate: "m/d/yyyy", shortTime: "h:MM TT", longTime: "h:MM:ss TT"
    }
    , dayNames: ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"], amPm: ["am", "pm", "AM", "PM"], s: function (a) {
...
ec.prototype = {
    begin: function () {
        ...
        for (var d = this.animations, g = new Date, b = false, c = 0;
            c < d.length;
            c++) {
...
function kc(e) {
    ...
    var f = {
        enabled: true, delayTime: 0, duration: 2;
    }
        ;
    if (!a) return;
...
xb.prototype = {
    ...
    , animate: function (a) {
        if (a >= this.endTime) {
            ...
            var c = a.getTime() - this.startTime.getTime(), b = this.from + this.valueDiff * c / this.timeDiff;
...
V.prototype.mouseDown = function (d) {
    for (var b = this.view.mouseDownTools, a = 0;
        a < b.length;
        a++) {
        var c = b[a];
        if (c.canStart(d)) {
...
V.prototype.mouseMove = function (d) {
    for (var b = this.view.mouseMoveTools, a = 0;
        a < b.length;
        a++) {
        var c = b[a];
        if (c.canStart(d)) {
...
V.prototype.touchStart = function (d) {
    for (var b = this.view.touchStartTools, a = 0;
        a < b.length;
        a++) {
        var c = b[a];
        if (c.canStart(d)) {
...
V.prototype.touchMove = function (d) {
    for (var b = this.view.touchMoveTools, a = 0;
        a < b.length;
        a++) {
        var c = b[a];
        if (c.canStart(d)) {
...
d.prototype = {
    _createElements: function (c) {
        ...
        if (d.isMouseDevice) {
            ...
            c.jqMouseCapture({
                down: a.proxy(b._mouseDown, this), move: a.proxy(b._mouseMove, this), up: a.proxy(b._mouseUp, this);
            }
            );
    ...
    , _createTooltip: function () {
        var b = a('<div style="position: absolute;
    display: none"></div>');
    this.getCanvasContainer().append(b);
        return b;
    }
    ...
    , _initTouchInput: function (j) {
        ...
        if (!this.locOffset) this.locOffset = this._getLocOffset();
        for (var c = this.locOffset, d = [], a = 0;
            a < b.length;
            a++) {
            var e = b[a], f = e.pageX, g = e.pageY, h = f - c.left, i = g - c.top;
            d.push({
                x: f, y: g, locX: h, locY: i;
            }
            );
        } this.touchInput = d;
    }, _getLocOffset: function () {
        var b = this.getCanvasContainer().offset(), c = parseFloat(a(document.body).css("borderLeftWidth"));
        if (!isNaN(c)) b.left += c;
    ...
    , getAllTouches: function (h) {
        for (var f = ["touches", "changedTouches"], b = [], c = 0;
            c < f.length;
            c++)for (var e = h.originalEvent[f[c]], d = 0;
                d < e.length;
                d++) {
                var g = e[d];
    ...
    , _processMouseEvents: function () {
        ...
        var e = this.hasTooltips, f = this.hasHighlighting;
        if (!e && !f) return;
        var g = this.options.tooltips.snapArea, a = this._getTooltipShapes(c.locX, c.locY, g, c), d = true;
    ...
    , _setTooltipPos: function (e, j) {
        var i = this.tooltip.outerWidth(true), h = this.tooltip.outerHeight(true), g = this._width, f = this._height, c = e[0]._getTooltipPosition(j, i, h, g, f), d = c.y, k = e.length, l = this;
        if (k > 1) {
            d = 0;
            a.each(e, function () {
    ...
    , _highlightShapes: function (e) {
        ...
        if (f.result !== false) if (d.use_excanvas) this.hlRenderer._render(b);
        else {
    ...
    , _getTooltipShapes: function (h, i, c, b) {
        if (!c) c = 0;
        for (var e = [], f = this.shapes.length - 1;
            f >= 0;
            f--) {
            var a = this.shapes[f];
    ...
    , _exportToImage: function (d, e) {
        ...
        if (d.fileName) c.fileName = b.replaceTextForExport(d.fileName);
            ...
            case "image/jpeg": c.fileName = "jqChart.jpg";
                break;
            case "image/png": default: c.fileName = "jqChart.png"
        }
        var f = this.toJSON(c);
...
, hitTest: function(d, e, b) {
    if (!b) b = 0;
    for (var c = this.shapes.length - 1;
        c >= 0;
        c--) {
        var a = this.shapes[c];
                ...
                if (e) {
                    for (var c = 0;
                        ...
                        c++)if (e[c].dataItem == d) {
                            ...
                            var f = {
                                x: this.pts[2 * c], y: this.pts[2 * c + 1];
                            }
                                ;
                            a.tooltipOrigin = f;
...
mb.prototype = {
    _initDefs: function () {
        a.extend(this, {
            maxInter200Px: 8, lblMargin: 4, crossOffsetX: 0, crossOffsetY: 0, origin: 0, length: 300, x: 0, y: 0;
        }
        );
        ...
        for (var k = h * this.maxInter200Px, e = Math.max(this.length * k / 200, 1), g = l - m, a = g / e, j = Math.pow(10, Math.floor(b.log10(a))), f = [10, 5, 2, 1], c = 0;
            c < f.length;
            c++) {
            var i = f[c], d = j * i;
            if (e < g / d) break;
    ...
    , _getIntervals: function (c, b) {
        ...
        for (var e = [], f = this._getIntervalStart(this._getActualVisibleMinimum(), c), a = f + d;
            a <= this._getActualVisibleMaximum();
    ...
    , _createLabel: function (c, d) {
        ...
        b.isAxisLabel = true;
        b.context = {
            chart: this.chart, axis: this, text: c;
        }
            ;
        a.extend(b, d);
    ...
    , _measureRotatedLabels: function (g) {
        for (var h = this.isAxisVertical, c = 0, b = 0, d = 0;
            d < g.length;
            d++) {
            var a = g[d], e = Math.sqrt(a.width * a.width + a.height * a.height), f = a.rotationAngle;
            if (h) {
        ...
        if (q) {
            for (var e = [], d = 0;
                ...
                switch (a.textBaseline) {
                    case "middle": k -= a.height / 2;
                        break;
                    case "bottom": k -= a.height;
                }
                for (var c = 0, c = 0;
                    c < e.length;
                    c++) {
                    ...
                    y: k, h: a.height, w: a.width + g, labels: [a];
                }
            ...
            for (var e = [], d = 0;
                ...
                var a = n[d], j = a.x;
                switch (a.textAlign) {
                    case "center": j -= a.width / 2;
                        break;
                    case "right": j -= a.width;
                }
                for (var f = false, c = 0, c = 0;
                    c < e.length;
                    c++) {
                    var b = e[c];
                    ...
                    x: j, w: a.width, h: a.height + g, labels: [a];
                }
        ...
        var f = 0, e = 0, h = 0, o = 0, l = 0, j = 0, n = 0, m = i, s = this.reversed === true, u = this.labels.position == "inside", r = this.isAxisVertical, g = this.lblMargin, k = 2 * g;
        if (r) for (var v = [], b = 0;
            b < c.length;
            b++) {
            var d = b;
            ...
            switch (a.textBaseline) {
                case "middle": q -= a.height / 2;
                    break;
                case "bottom": q -= a.height;
            }
            j = a.y;
            n = j + a.height + k;
            if (n < m) m = j;
            ...
            var a = c[d], p = a.x;
            switch (a.textAlign) {
                case "center": p -= a.width / 2;
                    break;
                case "right": p -= a.width;
            }
            h = a.x;
            o = h + a.width + k;
            if (h > l) l = o;
        ...
        var a = {
            ...
            ;
        if (this.labels) if (this.labels.angle) a = this._measureRotatedLabels(this._getLabels());
        else switch (this.labels.resolveOverlappingMode) {
            case "hide": a = this._removeOverlappedLabels(this._getLabels());
                break;
            case "multipleRows": default: a = this._correctLabelsPositions(this._getLabels());
        }
        this.title._measure();
        ...
        var a = this.x, d = this.y, c = this.x + this.width, e = this.y + this.height;
        switch (this.location) {
            case "left": c = a = this.x + this.width;
                break;
            case "right": c = a = this.x;
                break;
            case "top": e = d = this.y + this.height;
                break;
            case "bottom": e = d = this.y;
        }
        if (this.title.text) switch (this.location) {
            case "left": this.title.x = this.title.rotX = this.x;
                this.title.y = this.title.rotY = this.y + (this.height + this.title.width) / 2;
                this.title.rotationAngle = b.radians(-90);
                break;
            case "right": this.title.x = this.title.rotX = this.x + this.width;
                this.title.y = this.title.rotY = this.y + (this.height - this.title.width) / 2;
                this.title.rotationAngle = b.radians(90);
                break;
            case "top": this.title.x = this.x + (this.width - this.title.width) / 2;
                this.title.y = this.y;
                break;
            case "bottom": this.title.x = this.x + (this.width - this.title.width) / 2;
    ...
    , _updateOrigin: function () {
        ...
        if (this.labels && !this.labels.angle) switch (this.labels.resolveOverlappingMode) {
            case "hide": this._removeOverlappedLabels(c);
                break;
            case "multipleRows": default: this._correctLabelsPositions(c);
        }
        this._filterLabels(c);
        this.title._render(b);
        a.merge(b, c);
        return {
            postShapes: b, contextShapes: c;
        }
    }, _getMainLine: function () {
        var b = this.crossOffsetX, c = this.crossOffsetY, a = new h(this.x1 + b, this.y1 + c, this.x2 + b, this.y2 + c);
    ...
    , _getLabels: function () {
        ...
        if (!q) return [];
        for (var p = this._getLabelIntervals(q, c), r = p.length, t = c.showFirstLabel, u = c.showLastLabel, h = 0;
            h < r;
            h++) {
            if (!t && h == 0 || !u && h == r - 1) continue;
            var s = p[h], v = this.getLabel(s), a = this._createLabel(v, c), m = this.getPosition(s);
            switch (this.location) {
                case "left": if (g) a.x = this.x + this.width + e + i;
                else {
                    ...
                    a.y = m;
                    switch (c.vAlign) {
                        case "bottom": a.textBaseline = "top";
                            break;
                        case "top": a.textBaseline = "bottom"
                    }
                    if (this.labels.angle) {
                        var d = Math.min(90, Math.max(-90, this.labels.angle)), f = b.radians(d);
                        a.rotX = a.x;
                        a.rotY = a.y;
                        a.rotationAngle = f;
                    }
                ...
                case "top": a.x = m;
                    ...
                    switch (c.hAlign) {
                        case "center": a.textAlign = "center";
                            break;
                        case "left": a.textAlign = "right";
                            break;
                        case "right": a.textAlign = "left"
                    }
                    if (this.labels.angle) {
                        var d = Math.min(90, Math.max(-90, this.labels.angle));
                        a.flip = d > 0;
                        if (d > 0) d = -180 + d;
                        var f = b.radians(d), k = Math.sqrt(a.width * a.width + a.height * a.height);
        ...
        for (var c = this.chart.gridArea, i = c.x, j = c.y, h = c.width, g = c.height, d = b.length - 1;
            d >= 0;
            d--) {
            var e = b[d];
        ...
        for (var n = [], g = this.crossOffsetX, i = this.crossOffsetY, k = this.offset, j = c.position == "inside", r = this._getMarkInterval(c, p), b = c.length, o = this._getIntervals(r, c, p), d, f, e, a, m = 0;
            m < o.length;
            m++) {
    ...
    , getZoom: function () {
        ...
        var b = this._getActualMaximum() - this._getActualMinimum(), a = this._getActualVisibleMaximum() - this._getActualVisibleMinimum(), c = a / b;
        return c;
    }
    , setOptions: function (b) {
        ...
        if (c.majorGridLines) this.majorGridLines = new Wb(c.majorGridLines);
        if (c.minorGridLines) {
    ...
    , getPosition: function (f) {
        var e = this._getActualVisibleMaximum(), d = this._getActualVisibleMinimum(), a = this.length / (e - d) * (f - d), b = this.reversed === true, c = this.isAxisVertical;
        if (c && b === false || c === false && b) a = this.origin + this.length - a;
        else a += this.origin;
        return a;
    }
    , getValue: function (f) {
        var e = this._getActualVisibleMaximum(), b = this._getActualVisibleMinimum(), a = (f - this.origin) * (e - b) / this.length + b, c = this.reversed === true, d = this.isAxisVertical;
    ...
    , isInVisibleRange: function (d) {
        ...
        location: "left", labels: {
            visible: true, fillStyle: "black", lineWidth: 1, font: "11px sans-serif", position: "outside", showLastLabel: true, showFirstLabel: true, hAlign: "center", vAlign: "center"
        }
        , title: {
            text: undefined, font: "14px sans-serif", margin: 2;
        }
        , strokeStyle: "black", lineWidth: 1, margin: 5, visible: true, reversed: false, zoomEnabled: false;
    }
};
var l = -Number.MAX_VALUE, i = Number.MAX_VALUE;
function b() {
}
b.isNull = function (b) {
...
b.compareArrays = function (a, b) {
    ...
    for (var c = 0;
        c < a.length;
        c++)if (a[c] !== b[c]) return false;
    return true;
}
    ;
b.rotatePointAt = function (j, k, c, a, b) {
    var e = Math.sin(c), d = Math.cos(c), f = j - a, g = k - b, h = a + f * d - g * e, i = b + f * e + g * d;
    return {
        x: h, y: i;
    }
};
...
b.intersection = function (a, c, b, d) {
    var f, h = (d.x - b.x) * (a.y - b.y) - (d.y - b.y) * (a.x - b.x), i = (c.x - a.x) * (a.y - b.y) - (c.y - a.y) * (a.x - b.x), e = (d.y - b.y) * (c.x - a.x) - (d.x - b.x) * (c.y - a.y);
    if (e != 0) {
        var g = h / e, j = i / e;
        f = {
            x: a.x + g * (c.x - a.x), y: a.y + g * (c.y - a.y);
        }
    } return f;
};
b.processDataValue = function (a, b) {
    switch (b) {
        case "numeric": if (typeof a == "string") a = parseFloat(a);
            break;
        case "dateTime": if (typeof a == "string") a = new Date(a);
            break;
        case "string": if (typeof a != "string") a = a.toString();
    }
    return a;
};
b.processDataField = function (h, c) {
    ...
    for (var i, b, j = [], g = 0;
        ...
        if (d) b = d(b);
            ...
            case "numeric": if (typeof b == "string") b = parseFloat(b);
                break;
            case "dateTime": if (typeof b == "string") b = new Date(b);
        }
        j.push(b);
    } return j;
};
...
b.mergeArrays = function (f) {
    for (var h = f.length, c = [], a = 0;
        a < h;
        a++) {
        var e = f[a];
        if (e == null) continue;
        for (var i = c.length, b = 0;
            b < e.length;
            b++) {
            var g = e[b];
            if (b < i) {
...
b.calcNullValue = function (h, m, l, o) {
    ...
    } var j = b.isNull(c), i = b.isNull(d);
    if (j && i) return null;
    if (j) c = d;
    else if (i) d = c;
    var e;
    if (o == "DateTimeAxis") {
        var k = c.getTime(), n = d.getTime();
        e = k + (n - k) / (f - g);
        e = new Date(e);
    }
    else e = c + (d - c) / (f - g);
...
g.addMonths = function (c, b) {
    var a = new Date(c.getTime()), d = a.getDate();
    a.setDate(1);
...
c.compare = function (b, e, a, j) {
    ...
    var f = b.getCenter(a), g = e.getCenter(a), c = a.locX - f.x, d = a.locY - f.y, h = Math.sqrt(c * c + d * d);
    c = a.locX - g.x;
    d = a.locY - g.y;
...
c.prototype.isInRect = function (a, b, f, e) {
    var c = this.x, d = this.y, h = this.width, g = this.height;
...
c.prototype.renderDashedLine = function (e, g, f, h, k, a) {
    ...
    a.moveTo(e, g);
    var b = e, c = g, m = 0, l = true, t = k.length;
    while (!(i.thereYet(b, f) && j.thereYet(c, h))) {
        var p = Math.atan2(h - g, f - e), q = k[m];
...
c.prototype.calculateBounds = function (a) {
    if (a == null) return;
    for (var c = i, d = i, g = l, h = l, f, j, e = 0;
        e < a.length;
        e += 2) {
        f = a[e];
        j = a[e + 1];
...
c.prototype.fillStyleToJSON = function (a) {
    ...
    c += ", colorStops:[";
    for (var f = a.colorStops.length, d = 0;
        d < f;
        d++) {
...
c.prototype._createGradient = function (j, e) {
    ...
    var f, b = {
        x0: 0, y0: 0, x1: 1, y1: 1, r0: 0, r1: 1;
    }
        ;
    a.extend(b, e);
    switch (e.type) {
        case "radialGradient": var m = c + b.x0 * this.width, o = d + b.y0 * this.height, k = b.r0 * this.width / 2, n = c + b.x1 * this.width, p = d + b.y1 * this.height, l = b.r1 * this.width / 2;
            f = j.createRadialGradient(m, o, k, n, p, l);
            break;
        default: var s = c + b.x0 * this.width, t = d + b.y0 * this.height, r = c + b.x1 * this.width, q = d + b.y1 * this.height;
            f = j.createLinearGradient(s, t, r, q);
    }
    var g = b.colorStops;
    if (g != null) for (var h = 0;
        h < g.length;
        h++) {
        var i = g[h];
...
h.hitTest = function (a, b, c, d, o, p, g) {
    ...
    var r = Math.sqrt(e * e + f * f), q = (e * o - f * p + j) / r, s = Math.max(g, 3);
    return Math.abs(q) < s;
}
    ;
h.prototype.hitTest = function (f, g, j) {
    var b = this.x1, d = this.y1, c = this.x2, e = this.y2, a = Math.max(j, Math.max(3, this.lineWidth / 2));
    if (b == c) {
        var h = f + 1;
...
h.prototype.hitTestNonHV = function (n, o, l) {
    ...
    var k = Math.sqrt(e * e + f * f), j = (e * n - f * o + i) / k, m = Math.max(l, 3);
...
h.prototype.toJSON = function () {
    var a = this, b = "type: 'line', x1:" + a.x1 + ", y1:" + a.y1 + ", x2:" + a.x2 + ", y2:" + a.y2;
...
o.prototype.render = function (a, n) {
    ...
    var k = this.strokeStyle != null && this.lineWidth > 0, m = k && Math.floor(this.lineWidth % 2) ? b.roundH : Math.round, l = this.correction || 0;
    l = Math.round(l);
    var g = m(this.x), h = m(this.y), f = Math.round(this.width), e = Math.round(this.height), o = this.context;
    if (this.xDecrease) {
        f -= this.xDecrease;
    ...
    if (f <= 0 || e <= 0) return;
    var i = this.strokeDashArray, j = d.support_native_line_dash && i;
    j && a.setLineDash(i);
    if (this.cornerRadius == 0) this.renderRectPath(a, g, h, f, e);
    else this.renderRoundedRectPath(a, g, h, f, e);
...
o.prototype.toJSON = function () {
    var a = this, b = "type: 'rectangle', x:" + a.x + ", y:" + a.y + ", width:" + a.width + ", height:" + a.height;
...
Eb.prototype.hitTest = function (f, g, a) {
    ...
    var c = (this.width + a) / 2, b = (this.height + a) / 2, h = this.x + c, i = this.y + b, d = f - h, e = g - i, j = d * d / (c * c), k = e * e / (b * b);
    return j + k <= 1;
}
    ;
Eb.prototype.render = function (a, m) {
    ...
    var b = this.x, e = this.y, l = this.width, k = this.height;
    if (this.xOffset) b += this.xOffset;
    if (this.yOffset) e += this.yOffset;
    var g = this.width / 2 * .5522848, h = this.height / 2 * .5522848, i = b + l, j = e + k, f = b + l / 2, d = e + k / 2;
    a.beginPath();
    a.moveTo(b, d);
...
Eb.prototype.toJSON = function () {
    var a = this, b = "type: 'ellipse', x:" + a.x + ", y:" + a.y + ", width:" + a.width + ", height:" + a.height;
    b += c.prototype.toJSON.call(this);
    return b;
}
    ;
...
Fb.prototype.toJSON = function () {
    var a = this, b = "type: 'arc', x:" + a.x + ", y:" + a.y + ", radius:" + a.radius + ", startAngle:" + a.startAngle + ", endAngle:" + a.endAngle;
    b += c.prototype.toJSON.call(this);
    return b;
}
    ;
...
Z.prototype.hitTest = function (e, f, a) {
    if (!this.useHitTestArea) a = 0;
    var c = this.x + this.width / 2, d = this.y + this.height / 2, b = Math.pow(e - c, 2) + Math.pow(f - d, 2);
...
Z.prototype.toJSON = function () {
    var a = this, b = "type: 'circle', x:" + a.x + ", y:" + a.y + ", radius:" + a.radius;
    b += c.prototype.toJSON.call(this);
    return b;
}
    ;
function yb(f, g, h, a, b, d) {
    ...
    this.endAngle = d;
    this.width = this.height = 2 * a;
    var e = (b + d) / 2, i = f + a * Math.cos(e), j = g + a * Math.sin(e);
...
yb.prototype.hitTest = function (h, i) {
    var f = this.x, g = this.y, e = Math.pow(h - f, 2) + Math.pow(i - g, 2);
    if (e > Math.pow(this.outerRadius, 2) || e < Math.pow(this.innerRadius, 2)) return false;
    var j = f - h, k = g - i, a = Math.atan2(k, j) + Math.PI;
    a = b.normalizeAngle(a);
    var c = b.normalizeAngle(this.startAngle), d = b.normalizeAngle(this.endAngle);
...
yb.prototype.render = function (a, i) {
    ...
    a.beginPath();
    var f = Math.round(this.x), g = Math.round(this.y), b = this.startAngle, e = this.endAngle;
    if (d.use_excanvas) {
...
yb.prototype.toJSON = function () {
    var a = this, b = "type: 'pieSlice', x:" + a.x + ", y:" + a.y + ", radius:" + a.outerRadius + ", outerRadius:" + a.outerRadius + ", innerRadius:" + a.innerRadius + ", startAngle:" + a.startAngle + ", endAngle:" + a.endAngle;
    b += c.prototype.toJSON.call(this);
    return b;
}
    ;
...
A.prototype.hitTest = function (k, l, g) {
    ...
    for (var d = this.pts, m = Math.pow(g, 2), c = -1, f = i, h, j, b, n = this.isStepLine ? 4 : 2, a = 0;
        a < d.length;
        a += n) {
        h = d[a];
        j = d[a + 1];
...
A.prototype.createHighlightMark = function (c) {
    if (c == -1) return null;
    var d = this.context, g = this.pts, f = c / 2;
    if (this.isStepLine) f /= 2;
    var h = d.points[f], e = 5, b = new Z(g[c] - e, g[c + 1] - e, e);
    b.fillStyle = b.highlightingFillStyle = this.strokeStyle;
    b.strokeStyle = "white";
    b.lineWidth = 1;
    b.useHitTestArea = true;
    b.context = {
        series: d.series, chart: d.chart;
    }
        ;
...
A.prototype.getCenter = function (g) {
    ...
    if (!e || !e.points) return {
        ...
        ;
    for (var b = this.pts, a = 0, f = i, j = g.locX, d, h = this.isStepLine ? 4 : 2, c = 0;
        c < b.length;
        c += h) {
        d = Math.abs(b[c] - j);
        if (f > d) {
        ...
        x: b[a], y: b[a + 1], mark: this.createHighlightMark(a);
    }
};
...
q.prototype.renderPoints = function (a, c, j) {
    ...
    for (var i = this.nullHandling != "connect", f = true, b = 2;
        b < h;
        b += 2) {
        var e = c[b];
        if (e == null) {
...
q.prototype.render = function (e, j) {
    if (!this.visible) return;
    var g = this.strokeDashArray, h = d.support_native_line_dash && g;
    h && e.setLineDash(g);
    ...
    if (this.xOffset) {
        ...
        for (var f = 0;
            f < a.length;
            f += 2)a[f] += this.xOffset;
    }
    if (this.yOffset) {
        a = a.slice(0);
        for (var f = 1;
            f < a.length;
            f += 2)a[f] += this.yOffset;
    }
    e.beginPath();
    e.moveTo(a[0], a[1]);
...
q.prototype.renderDashed = function (k, f) {
    for (var j = f.length, i = this.nullHandling != "connect", h = this.strokeDashArray, c = false, d, e, a = 0;
        a < j;
        a += 2) {
        var b = f[a];
        if (b == null) {
...
q.prototype.toJSON = function () {
    var a = this, b = "type: 'polyline', pts:" + a.arrayToJSON(a.pts);
...
u.prototype.renderPoints = function (f, i) {
    ...
    if (h) {
        ...
        for (var c = 0;
            c < e;
            c += 2)d = d.concat(this.getControlPoints(b[c], b[c + 1], b[c + 2], b[c + 3], b[c + 4], b[c + 5], g));
        d = d.concat(d[0], d[1]);
        for (var c = 2;
            c < e + 2;
            c += 2)f.bezierCurveTo(d[2 * c - 2], d[2 * c - 1], d[2 * c], d[2 * c + 1], b[c + 2], b[c + 3]);
    }
    else {
        for (var c = 0;
            c < e - 4;
...
u.prototype.getControlPoints = function (d, f, a, b, e, g, j) {
    var h = Math.sqrt(Math.pow(a - d, 2) + Math.pow(b - f, 2)), k = Math.sqrt(Math.pow(e - a, 2) + Math.pow(g - b, 2)), c = j * h / (h + k), i = j - c, l = a + c * (d - e), m = b + c * (f - g), n = a - i * (d - e), o = b - i * (f - g);
    return [l, m, n, o];
}
    ;
u.prototype.toJSON = function () {
    var a = this, b = "type: 'curve', pts:" + a.arrayToJSON(a.pts);
    if (a.closed) b += ", closed: " + a.closed;
    b += c.prototype.toJSON.call(this);
    return b;
}
...
cb.prototype.toJSON = function () {
    var a = this, b = "type: 'area', pts:" + a.arrayToJSON(a.pts) + ", crossPos:" + a.crossPos + ", vertical:" + a.vertical;
...
Y.prototype.toJSON = function () {
    var a = this, b = "type: 'rangeShape', pts1:" + a.arrayToJSON(a.pts1) + ", pts2:" + a.arrayToJSON(a.pts2);
    if (a.isCurve) b += ", isCurve: " + a.isCurve;
    b += c.prototype.toJSON.call(this);
    return b;
}
...
R.prototype.hitTest = function (j, a, h) {
    var k = this.context;
    if (k && k.points) return A.prototype.hitTest.call(this, j, a, h);
    var n = this.boundsHitTest(j, a, h);
    if (n == false) return false;
    if (this.isBoundsHitTest && h) return true;
    for (var b = this.pts, g = false, l = b.length, i, c, m, e, d = 0, f = 0;
        f < l;
        f += 2) {
        d += 2;
        if (d == l) d = 0;
        ...
        m = b[d];
        e = b[d + 1];
        if (c < a && e >= a || e < a && c >= a) if (i + (a - c) / (e - c) * (m - i) < j) g = !g;
    }
    return g;
};
R.prototype.render = function (f, k) {
    ...
    var a = this.pts, j = a.length;
    if (j < 4) return;
    if (this.xOffset) {
        a = a.slice(0);
        for (var e = 0;
            e < a.length;
            e += 2)a[e] += this.xOffset;
    }
    if (this.yOffset) {
        a = a.slice(0);
        for (var e = 1;
            e < a.length;
            e += 2)a[e] += this.yOffset;
    }
    ...
    if (this.dontRound) g = function (a) {
        ...
        ;
    var h = this.strokeDashArray, i = d.support_native_line_dash && h;
    i && f.setLineDash(h);
    f.beginPath();
    f.moveTo(g(a[0]), g(a[1]));
    for (var e = 2;
        e < j;
        e += 2)f.lineTo(g(a[e]), g(a[e + 1]));
    f.closePath();
...
R.prototype.toJSON = function () {
    var b = this, a = "type: 'polygon', pts:" + b.arrayToJSON(b.pts);
...
k.prototype.render = function (a, d) {
    if (!this.visible) return;
    c.prototype.render.call(this, a, d);
    var e = b.roundH(this.x), f = b.roundH(this.y);
    if (this.rotationAngle && !b.isNull(this.rotX) && !b.isNull(this.rotY)) {
        a.save();
...
k.prototype.renderText = function (c, d, a) {
    var b = "" + this.text, i = b && b.search(k.separator) != -1;
    if (i) {
        var h = b.split(k.separator), e = this.getHeight(), f;
        switch (this.textBaseline) {
            case "middle": a -= (this.height - e) / 2;
                break;
            case "bottom": a -= this.height - e;
        }
        for (var g = 0;
            g < h.length;
            g++) {
...
k.prototype.getHeight = function () {
    for (var e = 0, c = this.font.split(" "), b = 0;
        b < c.length;
        b++) {
        var d = c[b], f = d.charAt(0);
...
k.prototype.measure = function (e) {
    ...
    var c = this.getHeight(), a, b, g = "" + this.text, h = g && g.search(k.separator) != -1;
    if (h) {
...
k.prototype.intersectWith = function (c, d, i, h) {
    var a = this.x, b = this.y, g = this.width, f = this.height, e = this._correctXY(a, b, g, f);
    a = e.x;
    b = e.y;
...
k.prototype.toJSON = function () {
    ...
    var e = b.replaceTextForExport(this.text), d = "type: 'textBlock', x:" + a.x + ", y:" + a.y + ", width:" + a.width + ", height:" + a.height + ", text:'" + e + "'";
    d += ", font: '" + a.font + "'";
...
ic.prototype.toJSON = function () {
    var a = this, b = "type: 'clip', x:" + a.x + ", y:" + a.y + ", width:" + a.width + ", height:" + a.height;
    return b;
}
    ;
function fc() {
...
ac.prototype.toJSON = function () {
    var b = this, a = "type: 'shadows', renderShadows:" + b.renderShadows;
    return a;
}
    ;
function P(a, b) {
...
function bb(a) {
    this.defaults = {
        text: "", font: "14px sans-serif", fillStyle: "black", lineWidth: 0, margin: 6;
    }
        ;
    this.x = 0;
    this.y = 0;
...
Vb.prototype._render = function (f) {
    ...
    var e = c._width, d = c._height, a = this.margin + this.textBlock.width / 2, b = this.margin;
    switch (this.hAlign) {
        case "center": a += (e - this.width) / 2;
            break;
        case "right": a += e - this.width;
    }
    switch (this.vAlign) {
        case "center": b += (d - this.height) / 2;
            break;
        case "bottom": b += d - this.height;
    }
...
function zb(a) {
    this.defaults = {
        visible: true, strokeStyle: "black", lineWidth: 1, lineCap: "butt", lineJoin: "miter", miterLimit: 10, cornerRadius: 10, padding: 4, ignoreOffset: false;
    }
        ;
    this.x = 0;
    this.y = 0;
...
zb.prototype._render = function (c) {
    var a = this.visible ? this.lineWidth / 2 : 0, f = this.x + a, g = this.y + a, e = this.width - 2 * a, d = this.height - 2 * a, b = new o(f, g, e, d);
...
function Xb(a) {
    this.defaults = {
        strokeStyle: "black", lineWidth: 1, length: 6, position: "outside", visible: true, zIndex: 2, offset: .4;
    }
        ;
    this.setOptions(a);
}
...
Nb.prototype.mouseWheel = function (e, c) {
    ...
    for (var b = 0;
        b < a.length;
        b++)if (d == "scrolling") a[b]._mouseWheelScroll(c);
...
X.prototype.mouseUp = function () {
    var a = this.mouseInput1, b = this.view.mouseInput, e = Math.abs(a.x - b.x), f = Math.abs(a.y - b.y);
    if (e > 1 || f > 1) {
        for (var d = this.zoomableAxes, c = 0;
            c < d.length;
            c++)d[c]._scaleToRegion(a, b);
        this.view.partialDelayedUpdate();
    }
    this.stopTool();
};
...
eb.prototype.mouseMove = function () {
    for (var f = this.zoomableAxes, a = this.view.mouseInput, c = this.zoomableAxes, d = this.oldMouseInput.locX - a.locX, e = this.oldMouseInput.locY - a.locY, b = 0;
        b < c.length;
        b++)c[b]._moveVisibleRange(d, e);
...
Q.prototype.canStart = function (d) {
    ...
    for (var b = this.view.touchInput, a = 0;
        a < b.length;
        a++) {
        var c = b[a];
...
Q.prototype.doPan = function () {
    for (var e = this.zoomableAxes, c = this.view.touchInput, d = c[0], b = this.oldTouchInput[0], f = b.locX - d.locX, g = b.locY - d.locY, a = 0;
        a < e.length;
        a++)e[a]._moveVisibleRange(f, g);
...
Q.prototype.doScale = function () {
    ...
    for (var d = this.getTwoTouchPointData(this.oldTouchInput), e = this.getTwoTouchPointData(a), c = this.zoomableAxes, b = 0;
        b < c.length;
        b++)c[b]._scaleVisibleRange(d, e);
    this.oldTouchInput = a;
    this.view.partialDelayedUpdate();
}
    ;
Q.prototype.getTwoTouchPointData = function (b) {
    var a = {
        x1: b[0].locX, y1: b[0].locY, x2: b[1].locX, y2: b[1].locY;
    }
        ;
...
e.prototype._setOptions = function (h) {
    ...
    c.title = a.extend({
        ...
        , a.fn.jqChart.defaults.title, c.title);
    c.crosshairs = a.extend(true, {
    }
        , a.fn.jqChart.defaults.crosshairs, c.crosshairs);
    c.tooltips = a.extend(true, {
    }
        , a.fn.jqChart.defaults.tooltips, c.tooltips);
    c.shadows = a.extend(true, {
    }
        , a.fn.jqChart.defaults.shadows, c.shadows);
    c.selectionRect = a.extend(true, {
    }
        , a.fn.jqChart.defaults.selectionRect, c.selectionRect);
    c.watermark = a.extend(true, {
    }
        , a.fn.jqChart.defaults.watermark, c.watermark);
    ...
    c.noDataMessage = a.extend(true, {
        ...
        , a.fn.jqChart.defaults.noDataMessage, c.noDataMessage);
    ...
    var f = this._width = this.elem.width(), e = this._height = this.elem.height(), c = this.options;
...
e.prototype._processAsyncData = function () {
    ...
    var d = this, i = d.options.dataSource;
    d.arrDataSource = null;
    var j = i.ajax, b = {
    }
        ;
    a.extend(b, j);
    b.url += "?";
    for (var h = this.axes.getZoomableAxes(), e = 0;
        e < h.length;
        e++) {
        var c = h[e];
...
e.prototype._arrange = function () {
    var d = this._width, g = this._height;
    this.border.width = d;
    this.border.height = g;
    var c = this.border.getPaddings();
    d -= c.left + c.right;
    g -= c.top + c.bottom;
    var e = c.left, f = c.top + this.title.height, o = this.axes._getTotalWidth(), n = this.axes._getTotalHeight();
    ...
    switch (this.legend.location) {
        case "left": e += this.legend.width;
            break;
        case "top": f += this.legend.height;
    }
    for (var l = this.axes.verCrossingAxis, k = this.axes.horCrossingAxis, i = this.axes._getAxesInLoc("left"), b = i.length - 1;
        b >= 0;
        b--) {
        var a = i[b];
        a.x = e;
    ...
    for (var j = this.axes._getAxesInLoc("right"), b = 0;
        b < j.length;
        b++) {
        var a = j[b];
        a.x = e;
    ...
    for (var p = this.axes._getAxesInLoc("top"), b = p.length - 1;
        b >= 0;
        b--) {
        var a = p[b];
        a.x = this.gridArea.x;
    ...
    for (var h = this.axes._getAxesInLoc("bottom"), b = 0;
        b < h.length;
        b++) {
        var a = h[b];
        a.x = this.gridArea.x;
    ...
    for (var m = i.concat(j), b = 0;
        b < m.length;
        b++) {
        var a = m[b];
        a.y = this.gridArea.y;
    }
    for (var h = this.axes._getAxesInLoc("radial"), b = 0;
        b < h.length;
        b++) {
        var a = h[b];
        a.x = this.gridArea.x;
    ...
    this.title.y = c.top;
    switch (this.legend.location) {
        case "bottom": this.legend.x = c.left + (d - this.legend.width) / 2;
            this.legend.y = c.top + g - this.legend.height;
            break;
        case "left": this.legend.x = c.left;
            this.legend.y = c.top + this.gridArea.y + (this.gridArea.height - this.legend.height) / 2;
            break;
        case "top": this.legend.x = c.left + (d - this.legend.width) / 2;
            this.legend.y = c.top + this.title.height;
            break;
        case "right": default: this.legend.x = c.left + d - this.legend.width;
...
e.prototype._getClosestShapeAtX = function (f, g) {
    for (var c = null, j = i, k = g.locX, d = f.length - 1;
        d >= 0;
        d--) {
        var a = f[d];
        if (!a.context || a.isLegendItem || a.isAxisLabel) continue;
        ...
        var b = a.getCenter(g), h = Math.abs(b.x - k);
        if (j > h) {
            j = h;
            c = a;
            if (b.mark) c = b.mark;
...
e.prototype._getClosestShapeAtY = function (c, d) {
    for (var b = null, g = i, j = d.locY, a = c.length - 1;
        a >= 0;
        a--) {
        var e = c[a], h = e.getCenter(d).y, f = Math.abs(h - j);
        if (g > f) {
            g = f;
            b = e;
        }
...
e.prototype._getShapesAtX = function (o, h, k, n) {
    for (var e = [h], j = [h.context.series], g = [0], i = 0;
        i < k.length;
        i++) {
        var b = k[i];
        if (b == h || !b.context || b.isLegendItem || b.isAxisLabel) continue;
        var l = b.context.series, c = a.inArray(l, j);
        if (c == 0) continue;
        var d = b.getCenter(n), f = Math.abs(o - d.x);
        if (f >= 3) continue;
        if (c != -1) {
            var m = g[c];
            if (f < m) {
...
e.prototype._getTooltipShapes = function (h, i, g, c) {
    ...
    if (b) {
        ...
        b.sort(function (d, e) {
            var b = a.inArray(d.context.series, series), c = a.inArray(e.context.series, series);
            return b - c;
        }
        );
    } return b;
...
e.prototype._initCrosshairs = function (d) {
    ...
    var h = this.options.crosshairs, c = {
    }
        , e = d.locX, f = d.locY;
...
e.prototype._renderSelectionRect = function (g, h) {
    var b = this.gridArea, c = b.fitHor(g.locX), e = b.fitVer(g.locY), d = b.fitHor(h.locX), f = b.fitVer(h.locY);
    if (!this.canZoomVer) {
        e = b.y;
        f = b.y + b.height;
    }
    ...
    var l = Math.min(c, d), m = Math.min(e, f), k = Math.abs(c - d), j = Math.abs(e - f), i = new o(l, m, k, j);
...
e.prototype._initZooming = function () {
    ...
    a.each(this.axes.items, function () {
        if (this.zoomEnabled) if (this.location != "radial") if (this.isAxisVertical) b.canZoomVer = true;
...
e.prototype._createShapes = function () {
    ...
    if (this.hasData) {
        ...
        var h = b.serShapes = [], j = f._renderSeries(h);
        b.gaZIndexShapes = [];
...
e.prototype._renderShapes = function () {
    var j = d.isRetinaDisplay, f = this._width, e = this._height;
    if (j) {
    ...
    var k = this.gridArea, b = this.allShapes;
...
e.prototype.toJSON = function (j) {
    var e = this.gridArea, d = this.options.shadows, f = d && d.enabled, i = new ic(e.x, e.y, e.width, e.height), h = new fc, d = new ac(true), g = new ac(false), c = this.allShapes, b = [];
    a.merge(b, c.shapes);
    if (this.hasData) {
        b.push(i);
        if (f) {
...
N.prototype.getTooltipColor = function () {
    if (this.isUp) if (this.priceUpStrokeStyle) return c.getColorFromFillStyle(this.priceUpStrokeStyle);
...
N.prototype.toJSON = function () {
    var a = this, b = "type: 'stock', x:" + a.x + ", width:" + a.width + ", high:" + a.high + ", low:" + a.low + ", open:" + a.open + ", close:" + a.close;
...
ib.prototype.createElements = function (b, e, f, n, l, m) {
    var d = [], g = Math.floor(m / 2), b = Math.round(b), a = Math.round(n), c = Math.round(l);
    if (a > c) {
        var k = c;
        c = a;
        a = k;
...
ib.prototype.render = function (d) {
    ...
    for (var b = 0;
        ...
        var a = this.items[b];
        this.setProperties(a);
        if (a instanceof o) if (this.isUp) a.fillStyle = this.priceUpFillStyle;
...
ib.prototype.toJSON = function () {
    var a = this, b = "type: 'candlestick', x:" + a.x + ", width:" + a.width + ", high:" + a.high + ", low:" + a.low + ", open:" + a.open + ", close:" + a.close;
...
Yb.prototype.getColors = function (a) {
    switch (a.toLowerCase()) {
        case "customcolors": return this.customColors;
...
function cc(a) {
    this.chart = a;
    this.defaults = {
        visibility: "auto", resetZoomTooltipText: "Reset Zoom (100%)", zoomingTooltipText: "Zoom in to selection area", panningTooltipText: "Pan the chart", hAlign: "right", vAlign: "top", resetZoomButtonVisible: true, zoomingButtonVisible: true, panningButtonVisible: true;
    }
} cc.prototype = {
    ...
    , _init: function () {
        var b = this.toolbar, d = this.chart;
        if (this.visibility == "hidden" || this.visibility == "auto" && !d.canZoom) {
            ...
            this.toolbar = b = a('<ul onselectstart="return false;
    " class="ui- jqchart - toolbar ui- widget ui- widget - content ui- corner - all"></ul>');
    this._addButtons();
            b.hide();
            d.elem.append(b);
        ...
        } var h = b.outerWidth(), g = b.outerHeight(), c = d.gridArea, e = c.x, f = c.y;
        switch (this.hAlign) {
            case "center": e += (c.width - h) / 2;
                break;
            case "right": e += c.width - h;
        }
        switch (this.vAlign) {
            case "center": f += (c.height - g) / 2;
                break;
            case "bottom": f += c.height - g;
        }
        b.css({
            left: e, top: f;
        }
    ...
    , _addButtons: function () {
        var f = this.toolbar, d = this.chart;
        ...
        if (this.panningButtonVisible) {
            ...
            case "zooming": b && b.addClass("ui-state-active");
                break;
            default: c && c.addClass("ui-state-active");
        }
...
function ab(a) {
    this.defaults = {
        lineCap: "butt", lineJoin: "miter", miterLimit: 10, lineWidth: 1, size: 8, offset: 0, linkLineWidth: 1, type: "circle"
    }
        ;
    this.setOptions(a);
}
...
ab.prototype.getShape = function (c, d, b, j, f) {
    ...
    f = f || this.type;
    switch (f) {
        case "circle": e = new Z(c - b, d - b, b);
            break;
        case "rectangle": e = new o(c - b, d - b, i, i);
            break;
        case "diamond": var a = [];
            a.push(c);
            a.push(d - b);
...
function Wb(a) {
    this.defaults = {
        strokeStyle: "gray", lineWidth: 1, visible: true;
    }
        ;
    this.setOptions(a);
}
...
function hc(a) {
    this.defaults = {
        strokeStyle: "gray", lineWidth: 1, zIndex: 0, title: {
            margin: 2, hAlign: "left", vAlign: "top"
        }
    };
    this.setOptions(a);
}
...
function gc(a) {
    this.defaults = {
        lineWidth: 0, fillStyle: "gray", zIndex: 0, title: {
            margin: 2, hAlign: "left", vAlign: "top"
        }
    };
    this.setOptions(a);
...
function nb(a) {
    this.defaults = {
        ...
        , border: {
            padding: 2, strokeStyle: "gray", cornerRadius: 6;
        }
        , margin: 4, visible: true, font: "12px sans-serif", textLineWidth: 0, inactiveTextLineWidth: 0, itemsHorMargin: 4, itemsVerMargin: 0, allowHideSeries: true, inactiveTextFillStyle: "gray", inactiveFillStyle: "gray", inactiveStrokeStyle: "gray"
    };
    this.setOptions(a);
}
...
nb.prototype._init = function () {
    ...
    if (this.customItems) for (var d = 0;
        d < this.customItems.length;
        d++) {
        ...
        var j = {
            marker: {
                type: "rectangle", fillStyle: "#418CF0"
            }
        };
        b = a.extend(true, {
        }
        ...
        for (var i = {
            font: this.font, textStrokeStyle: this.textStrokeStyle, textFillStyle: this.textFillStyle, textLineWidth: this.textLineWidth, cursor: e, inactiveTextFillStyle: this.inactiveTextFillStyle, inactiveTextStrokeStyle: this.inactiveTextStrokeStyle, inactiveTextLineWidth: this.inactiveTextLineWidth, inactiveFillStyle: this.inactiveFillStyle, inactiveStrokeStyle: this.inactiveStrokeStyle;
        }
            , g = this.chart.series.items, d = 0;
            d < g.length;
            d++) {
            var h = g[d];
...
nb.prototype._measure = function () {
    ...
    this.title._measure();
    var j = this.title.width, i = this.title.height;
    if (this.title.text) i += e.top;
    for (var l = this._isHorizontal(), g = this.itemsHorMargin, p = this.itemsVerMargin, b = 0, a = 0, d = 0, f = 0;
        f < this.items.length;
        f++) {
        var c = this.items[f];
        c._measure();
    ...
    } var o = 2 * this.margin + e.left + e.right, q = 2 * this.margin + e.top + e.bottom, k = this.chart.border.getPaddings(), m = this.chart._width - k.left - k.right - o;
    if (l && b > m) {
        var h = Math.floor(m / d) || 1, n = Math.ceil(this.items.length / h);
        this.rows = h;
        this.cellHeight = a + g;
        this.cellWidth = d;
        b = h * d;
...
nb.prototype._arrange = function () {
    if (this.visible == false) return;
    var e = this.x + this.margin, b = this.y + this.margin, h = 2 * this.margin;
    this.border.x = e;
    this.border.y = b;
    var g = this.width - h;
    this.border.width = g;
    ...
    for (var i = this._isHorizontal(), f = this.rows, d = 0;
        d < this.items.length;
        d++) {
        var a = this.items[d];
        a.x = e;
...
nb.prototype._handleEvent = function (e, d) {
    ...
    var a = d.context, c = a.series, b = a.chart;
    if (!c) return;
    switch (e) {
        case "MouseDown": case "TouchEnd": c._hideFromLegend(a);
            var f = b.mouseInput;
            b.update();
...
function wb(a) {
    this.defaults = {
        font: "12px sans-serif", textFillStyle: "black", textLineWidth: 0;
    }
        ;
    this.lblMargin = 4;
    this.setOptions(a);
...
wb.prototype._arrange = function () {
    var a = this.marker.size / 2, b = this.x + a, c = this.y + a + (this.height - this.marker.size) / 2;
...
C.prototype._arrangeRenderer = function (a) {
    if (!d.use_excanvas) return;
    var f = this.x, g = this.y, b = a.canvas, h = a.div, e = Math.max(this.width, 0), c = Math.max(this.height, 0);
    h.css({
        left: f, top: g, width: e, height: c;
    }
    );
    b.width = e;
    b.height = c;
...
C.prototype._renderSeries = function (g) {
    for (var e = this.chart.series.items, d = [], b = 0;
        b < e.length;
        b++) {
        var f = e[b];
        if (!f.notInGridArea) {
...
C.prototype._renderStripes = function (e) {
    for (var c = this.chart.axes.items, b = 0;
        b < c.length;
        b++) {
...
C.prototype._renderPlots = function (f, g) {
    for (var d = [], h = this.chart.axes.items, e = 0;
        e < h.length;
        e++) {
        var i = h[e], b = this._getPlotBands(i, g);
        if (b) {
            a.merge(f, b.shapes);
            a.merge(d, b.texts);
        }
...
C.prototype._renderGridLines = function (e) {
    for (var f = this.chart.axes.items, d = 0;
        d < f.length;
        d++) {
...
C.prototype._getStripes = function (b) {
    ...
    var c = this, e = [];
    a.each(d, function () {
        var a = new Stripe(this), j;
        if (a.interval) j = a.interval;
        else j = 2 * b.actualInterval;
        var i;
        if (a.width) i = a.width;
        else i = b.actualInterval;
        for (var h = a.lineWidth, n = h / 2, m = b._getIntervals(j, a, true), l = 0;
            l < m.length;
            l++) {
            var k = m[l];
            if (k >= b.actualVisibleMaximum) continue;
            var d = b.getPosition(k), p = b._getNextPosition(k, i), f = b.getPosition(p), g;
...
C.prototype._getGridLines = function (a, b, k) {
    ...
    for (var i = [], n = a._getMarkInterval(b, k), j = a._getIntervals(n, b, k), c, d, e, f, g = 0;
        g < j.length;
        g++) {
...
C.prototype._getPlotLines = function (i, o) {
    ...
    for (var e = this, k = [], l = [], j = 0;
        j < g.length;
        j++) {
        var d = new hc;
        if (!i.isAxisVertical) d.defaults.title.hAlign = "right";
        d.setOptions(g[j]);
        if (b.isNull(d.value)) continue;
        if (b.fitInRange(d.zIndex, 0, 2) != o) continue;
        var f = d.lineWidth / 2, n = i.getPosition(d.value), c = new bb(d.title);
        c.chart = this.chart;
        c._measure();
        if (i.isAxisVertical) {
            y1 = y2 = n;
            ...
            c.y = y1;
            switch (c.hAlign) {
                case "center": c.x = e.x + (e.width - c.width) / 2;
                    break;
                case "right": c.x = e.x + e.width - c.width;
            }
            switch (c.vAlign) {
                case "bottom": c.y += f;
                    break;
                case "center": c.y -= c.height / 2;
                    break;
                case "top": c.y -= c.height + f;
            }
        } else {
            x1 = x2 = n;
            y1 = this.y;
            ...
            switch (c.hAlign) {
                case "right": c.x += f;
                    break;
                case "center": c.x -= c.height / 2;
                    break;
                case "left": c.x -= c.height + f;
            }
            switch (c.vAlign) {
                case "center": c.y += (e.height - c.width) / 2;
                    break;
                case "bottom": c.y += e.height - c.width;
            }
...
C.prototype._getPlotBands = function (g, u) {
    ...
    for (var k = this, q = [], r = [], p = 0;
        ...
        var n = d.lineWidth, t = n / 2, l = g.getPosition(d.from), m = g.getPosition(d.to), h, i, f, e;
        if (g.isAxisVertical) {
            h = k.x + t;
            i = Math.min(l, m);
            f = k.width - n;
        ...
        if (g.isAxisVertical) {
            ...
            c.y = i;
            switch (c.hAlign) {
                case "center": c.x += (f - c.width) / 2;
                    break;
                case "right": c.x += f - c.width;
            }
            switch (c.vAlign) {
                case "center": c.y += (e - c.height) / 2;
                    break;
                case "bottom": c.y += e - c.height;
            }
        } else {
            c.x = h;
            c.y = i;
            switch (c.hAlign) {
                case "center": c.x += (f - c.height) / 2;
                    break;
                case "right": c.x += f - c.height;
            }
            switch (c.vAlign) {
                case "center": c.y += (e - c.width) / 2;
                    break;
                case "bottom": c.y += e - c.width;
            }
            c.rotX = c.x + c.height - c.margin;
            c.rotY = c.y + c.margin;
            c.rotationAngle = b.radians(90);
...
y.prototype.setOptions = function (e) {
    ...
    for (var f = 0;
        ...
        d = d.toLowerCase();
        switch (d) {
            case "area": b = new fb(c);
                break;
            case "splinearea": b = new Bb(c);
                break;
            case "bar": b = new s(c);
                break;
            case "bubble": b = new db(c);
                break;
            case "line": b = new B(c);
                break;
            case "stepline": b = new Sb(c);
                break;
            case "steparea": b = new Rb(c);
                break;
            case "spline": b = new Ub(c);
                break;
            case "pie": b = new p(c);
                break;
            case "donut": case "doughnut": b = new Zb(c);
                break;
            case "scatter": b = new Tb(c);
                break;
            case "stackedcolumn": b = new L(c);
                break;
            case "stackedbar": b = new I(c);
                break;
            case "stacked100column": b = new gb(c);
                break;
            case "stacked100bar": b = new tb(c);
                break;
            case "rangecolumn": b = new H(c);
                break;
            case "rangebar": b = new t(c);
                break;
            case "gantt": b = new W(c);
                break;
            case "stock": b = new G(c);
                break;
            case "candlestick": b = new Ab(c);
                break;
            case "radar": case "radarline": b = new J(c);
                break;
            case "radararea": b = new Qb(c);
                break;
            case "radarspline": b = new Lb(c);
                break;
            case "radarsplinearea": b = new Hb(c);
                break;
            case "polar": case "polarline": b = new E(c);
                break;
            case "polararea": b = new Pb(c);
                break;
            case "polarspline": b = new Kb(c);
                break;
            case "polarsplinearea": b = new Gb(c);
                break;
            case "polarscatter": b = new Jb(c);
                break;
            case "trendline": b = new U(c);
                break;
            case "verticalline": b = new O(c);
                break;
            case "verticalspline": b = new Ob(c);
                break;
            case "verticalarea": b = new jb(c);
                break;
            case "verticalsplinearea": b = new Ib(c);
                break;
            case "range": b = new z(c);
                break;
            case "splinerange": b = new Mb(c);
                break;
            case "stackedline": b = new v(c);
                break;
            case "stackedspline": b = new hb(c);
                break;
            case "stacked100line": b = new sb(c);
                break;
            case "stacked100spline": b = new qb(c);
                break;
            case "stackedarea": b = new T(c);
                break;
            case "stackedsplinearea": b = new pb(c);
                break;
            case "stacked100area": b = new rb(c);
                break;
            case "stacked100splinearea": b = new ob(c);
                break;
            case "funnel": b = new kb(c);
                break;
            case "column": default: b = new K(c);
        }
        b.type = d;
        b.chart = this.chart;
        this.items.push(b);
...
y.prototype._initCategories = function () {
    for (var d = [], e = 0, a = 0, f = this.items, c = 0;
        c < f.length;
        c++) {
        var b = f[c];
        if (!b.isInScene() || !b.categories) continue;
        for (a = e;
            a < b.categories.length;
            a++) {
...
y.prototype._initRanges = function () {
    for (var e = i, d = l, c = i, b = l, g = this.items, f = 0;
        f < g.length;
        f++) {
        var a = g[f];
...
y.prototype._findClusters = function (g, f) {
    for (var d = -1, a = 0, e = this.items, c = 0;
        c < e.length;
        c++) {
        var b = e[c];
...
y.prototype._findStackedClusters = function (h, e) {
    for (var f = h.stackedGroupName, g = this._getStackedGroupsFromType(e), l = a.inArray(f, g), k = g.length, i = -1, c = 0, j = this.items, d = 0;
        d < j.length;
        d++) {
        var b = j[d];
        if (!b.isInScene()) continue;
    ...
    return {
        index: i, count: c, groupIndex: l, groupCount: k;
    }
};
y.prototype._getSeriesFromType = function (e) {
    for (var c = [], d = this.items, b = 0;
        b < d.length;
        b++) {
        var a = d[b];
        if (!a.isInScene()) continue;
...
y.prototype._initColors = function () {
    for (var b = this.chart.paletteColors, c = this.items, a = 0;
        a < c.length;
        a++) {
        var d = c[a];
...
y.prototype._getPixelMargins = function (g) {
    for (var b = 0, a = 0, e = this.items, c = 0;
        c < e.length;
        c++) {
        var f = e[c];
...
f.prototype = {
    _getMarker: function (j, k, c, l, h, i, g) {
        ...
        var e = this.hasNullValues && a.inArray(i, this.nullIndexes) != -1, d = this.emptyPointStyleObj.marker, f;
        if (e) {
        ...
        var a = this._correctMarkerPosition(d, e, c), i = this._getMarker(a.x, a.y, l, c, f, k, g), j = this.markers.offset, b = null;
        if (j) {
            b = new h(d, e, a.x, a.y);
        ...
        return {
            marker: i, line: b, offset: a.offset;
        }
        ...
        var j = 0, q = !b.isNull(e) ? e : a;
        if (!c) c = {
            chart: this.chart, series: this, dataItem: this.arrData[a], index: a, x: this.realXAxis._getValue(q), y: f;
        }
            ;
        if (this.markers && this.markers.isVisible() && this.realYAxis.isValueVisible(f)) {
            var n = c.dataItem[2], d = this._addMarker(l, m, null, f, n, a, c);
            if (d.marker) {
    ...
    , _addShapeAnimation: function (e, g, f) {
        ...
        var a = new Cb(b, e, "visible", false, true), c = a.duration / f, d = a.delayTime + g * c;
        a.delayTime = d;
        a.duration = c;
    ...
    , _setMarkerSettings: function (a) {
        ...
        var c = this.options;
        if (!c.markers || b.isNull(c.markers.lineWidth)) if (!b.isNull(c.lineWidth)) a.lineWidth = c.lineWidth;
    ...
    , _setShapeSettings: function (c, e) {
        ...
        for (var e = 0;
            ...
            var g = a.type(f);
            switch (g) {
                case "number": b = "LinearAxis";
                    break;
                case "date": b = "DateTimeAxis";
                    break;
                case "string": b = "CategoryAxis";
                    break;
                default: b = "none"
            }
            break;
        ...
        switch (this.xAxisType) {
            case "LinearAxis": b.type = "linear";
                break;
            case "DateTimeAxis": b.type = "dateTime";
                break;
            case "CategoryAxis": b.type = "category"
        }
    }, _processData: function () {
        this.arrData = null;
        if (this.data) if (!this.xValuesType && !this.yValuesType) this.arrData = b.cloneArray(this.data);
        else {
            this.arrData = [];
            for (var d = 0;
                d < this.data.length;
            ...
            if (c) {
                var f = this.xValuesField, g = this.yValuesField, h = b.processDataField(c, f), e = b.processDataField(c, g);
    ...
    , _processErrorBars: function () {
        ...
        if (a && this.hasErrorBars && this.arrData) this.errorBarsObj = new lb(this, a);
    }
    , _processNullValues: function () {
        ...
        for (var i = this.nullIndexes = [], f, e, g, h, d, a = 0;
            a < c.length;
            a++) {
            f = c[a];
            e = b.isNull(f);
        ...
        this.arrData = null;
        if (this.data) if (!this.xValuesType && !this.fromValuesType && !this.toValuesType) this.arrData = b.cloneArray(this.data);
        else {
            this.arrData = [];
            for (var d = 0;
                d < this.data.length;
            ...
            var c = this.chart.arrDataSource;
            if (c) {
                var k = this.xValuesField, i = this.fromValuesField, j = this.toValuesField, h = this.labelValuesField, g = b.processDataField(c, k), e = b.processDataField(c, i), f = b.processDataField(c, j), l = b.processDataField(c, h);
    ...
    , _initXYData: function () {
        for (var m = this.arrData, d = i, c = l, h = i, g = l, n = m.length, k = 0;
            k < n;
            k++) {
            var j = m[k];
            if (j == null) continue;
    ...
    , _initCatValueData: function () {
        for (var k = this.arrData, h = i, g = l, f = [], m = k.length, e = 0;
            e < m;
            e++) {
            var d = k[e];
            if (d == null) {
    ...
    , _initDateValueData: function () {
        for (var m = this.arrData, e = i, d = l, j = i, h = l, n = m.length, f, a, c, k = 0;
            k < n;
            k++) {
            f = m[k];
            if (f == null) continue;
    ...
    , _initXYDataRange: function (n, p) {
        for (var m = this.arrData, h = i, g = l, f = i, e = l, o = m.length, j = 0;
            j < o;
            j++) {
            var d = m[j];
    ...
    , _initCatValueDataRange: function (o, q, n) {
        for (var k = this.arrData, g = i, f = l, d = [], p = k.length, e = 0;
            e < p;
            e++) {
            var h = k[e];
            if (h == null) {
    ...
    , _initDateValueDataRange: function (o, q) {
        for (var n = this.arrData, j = i, h = l, g = i, f = l, p = n.length, k = 0;
            k < p;
            k++) {
            var e = n[k];
    ...
    , _initData: function () {
        switch (this.xAxisType) {
            case "LinearAxis": this._initXYData();
                return;
        ...
        switch (this.xAxisType) {
            case "LinearAxis": case "DateTimeAxis": this._initVisibleXYData();
                return;
            case "CategoryAxis": this._initVisibleCatValueData();
                return;
        }
    }, _initVisibleXYData: function () {
        if (this.realYAxis.zoomEnabled) return;
        var c = this.realXAxis, o = c.visibleMinimum || c.minimum, n = c.visibleMaximum || c.maximum, k = !b.isNull(o), j = !b.isNull(n);
        if (!k && !j) return;
        for (var m = this.arrData, e = i, d = l, q = m.length, h = 0;
            h < q;
            h++) {
            var g = m[h];
            if (g == null) continue;
    ...
    , _initVisibleCatValueData: function () {
        if (this.realYAxis.zoomEnabled) return;
        var e = this.realXAxis, p = e.visibleMinimum || e.minimum, o = e.visibleMaximum || e.maximum, m = !b.isNull(p), k = !b.isNull(o);
        if (!m && !k) return;
        for (var n = this.arrData, g = i, f = l, q = n.length, d = 0;
            d < q;
            d++) {
            var h = n[d];
            if (h == null) continue;
    ...
    , _initVisibleCatValueDataRange: function (p, r) {
        if (this.realYAxis.zoomEnabled) return;
        var d = this.realXAxis, n = d.visibleMinimum || d.minimum, m = d.visibleMaximum || d.maximum, j = !b.isNull(n), h = !b.isNull(m);
        if (!j && !h) return;
        for (var k = this.arrData, f = i, e = l, q = k.length, c = 0;
            c < q;
            c++) {
            var o = k[c];
            if (o == null) continue;
    ...
    , _initVisibleXYDataRange: function (q, s) {
        ...
        var c = this.realXAxis, o = c.visibleMinimum || c.minimum, n = c.visibleMaximum || c.maximum, k = !b.isNull(o), j = !b.isNull(n);
        if (!k && !j) return;
        for (var m = this.arrData, e = i, d = l, r = m.length, g = 0;
            g < r;
            g++) {
            var f = m[g];
            if (f == null) continue;
    ...
    , _initStackedData: function (b) {
        ...
        var f = this.chart.series._findStackedClusters(this, b), e = this.chart.series._getStackedSeriesFromType(b, this.stackedGroupName), d = this._calcStackedData(c, f, e);
        a.extend(this, d);
    }
    , _initVisibleStackedData: function (b) {
        var c = this.arrData;
        if (a.isArray(c) == false) return;
        var f = this.chart.series._findStackedClusters(this, b), e = this.chart.series._getStackedSeriesFromType(b, this.stackedGroupName), d = this._calcVisibleStackedData(c, f, e);
        a.extend(this, d);
    }
    , _calcStackedData: function (t, v, u) {
        for (var o = i, n = l, m = i, k = l, h = [], w = t.length, r = {
        }
            , g = 0;
            g < w;
            g++) {
            ...
            if (this.xAxisType == "CategoryAxis") if (a.isArray(d) == false) h.push((g + 1).toString());
            else h.push(d[0]);
            else {
                var e = d[0];
        ...
        if (!g) return;
        var m = g.visibleMinimum || g.minimum, k = g.visibleMaximum || g.maximum, j = !b.isNull(m), h = !b.isNull(k);
        if (!j && !h) return;
        for (var o = i, n = l, x = s.length, q = {
        }
            , f = 0;
            f < x;
            f++) {
    ...
    , _createXAxis: function () {
        ...
        var b = {
            location: "bottom", orientation: "x"
        }
            , a;
        switch (this.xAxisType) {
            case "DateTimeAxis": a = new j(b);
                break;
            case "CategoryAxis": a = new F(b);
                break;
            default: a = new n(b);
        }
        a.chart = this.chart;
    ...
    , _initXAxis: function (b) {
        ...
        switch (this.type) {
            case "line": case "trendline": c.type = "line";
                c.lineWidth = this.lineWidth;
                break;
            case "scatter": case "bubble": if (this.markers) c.type = this.markers.type;
        }
        var j = {
            chart: this.chart, series: this;
        }
            , e = a.extend(false, {
            }
                , d, {
                    context: j, text: g, marker: c;
                }
            );
        if (!this.showInScene) {
    ...
    , _getPixelMargins: function (d) {
        ...
        if (this.labels && this.labels.visible !== false) {
            ...
            ;
        var f = d.isVertical(), l = this.isVertical, b;
    ...
    , _getLabelValue: function (a, b) {
        switch (this.labels.valueType) {
            case "percentage": a = this.getPercentage(a, b);
        }
        return a;
    }, _getDataPointLabel: function (i, j, e, c, f) {
        var g = i >= this.realYAxis.getCrossing(), h = this._getLabelText(f), b = new k(h);
        ...
        for (var f = this.chart.series._getSeriesFromType(h), e = 0, d = 0, c = 0;
            c < f.length;
            c++) {
            var j = f[c], g = j.arrData;
            if (g == null) continue;
            var b = g[i];
        ...
        return {
            positive: e, negative: d;
        }
    }, _getStackedTotal: function (f, g) {
        for (var e = 0, d = 0, b = 0;
            b < f.length;
            b++) {
            var h = f[b], c = h.dataValues;
            if (!c) continue;
            var a = c[g];
            if (!a) continue;
        ...
        if (this.xAxisType == "CategoryAxis") if (a.isArray(c) == false) b = (d + 1).toString();
        else b = c[0];
        else {
            b = c[0];
    ...
    , _getTooltip: function (b) {
        ...
        for (var h = [], d = 0;
            d < m.length;
            d++)a.merge(h, m[d].arrData || []);
        h.sort(function (a, b) {
            return !a || !b ? 0 : a[0] - b[0];
        }
        );
        for (var f, g, e, k = i, d = 0;
            d < h.length - 1;
            d++) {
            e = h[d];
            if (!e) continue;
        ...
        var c = this.realXAxis, l = c.series, o = b.isNull(c.minimum) ? l.minX : c.minimum, n = b.isNull(c.maximum) ? l.maxX : c.maximum, j = n - o;
    ...
    , _render: function (a) {
        ...
        switch (this.xAxisType) {
            case "LinearAxis": case "DateTimeAxis": return this._renderLinearData(a);
            case "CategoryAxis": return this._renderCatData(a);
        }
    ...
    , getPercentage: function (a, d) {
        var b = this._getTotal(this.type, d), c = a > 0 ? b.positive : b.negative;
    ...
    , setOptions: function (c) {
        ...
        a.extend(this, b);
        this.options = c;
        if (c) this.emptyPointStyleObj = new bc(b.emptyPointStyle);
        if (b.markers != null) this.markers = new ab(b.markers);
    }
    , isInScene: function () {
    ...
    , getLabelsOffset: function () {
        var c = this.markers && this.markers.isVisible(), a = c ? this.markers.size / 2 : 0, b = this.errorBarsObj;
        if (b) a = Math.max(a, b.getMaxLength());
        return a;
    }
    , defaults: {
        lineCap: "butt", lineJoin: "round", miterLimit: 10, pointWidth: .6, minimumWidth: 1, nullHandling: "break", visible: true, lineWidth: 1, strokeStyle: null, showInLegend: true, showInScene: true;
    }
};
function B(b) {
    var c = a.extend(true, {
...
B.prototype._renderCatData = function (n) {
    for (var o = this.arrData, p = o.length, h = this.markers != null && this.markers.isVisible(), c = [], i = [], m = [], k = [], q = this.getLabelsOffset(), g, j, r, d, e = 0;
        e < p;
        e++) {
        var f = o[e];
        if (f === null) {
        ...
        c.push(j);
        !h && i.push({
            dataItem: f, index: e, x: this.realXAxis._getValue(e), y: d;
        }
        );
    ...
    if (!h && l) l.context = {
        chart: this.chart, series: this, points: i;
    }
        ;
    else a.merge(n, m);
    return k;
...
B.prototype._renderLinearData = function (j) {
    var q = this.arrData, k = q.length, e = this.markers != null && this.markers.isVisible(), r = this.labels != null && this.labels.visible !== false;
    if (k > 1e3 && e == false && r == false) {
        this._renderLargeXYData(j);
        return;
    }
    for (var c = [], h = [], p = [], n = [], s = this.getLabelsOffset(), l, m, f, d, g = 0;
        g < k;
        g++) {
        var i = q[g];
        if (i === null) {
    ...
    if (!e && o) o.context = {
        chart: this.chart, series: this, points: h;
    }
        ;
    else a.merge(j, p);
    return n;
...
B.prototype._renderLargeXYData = function (u) {
    for (var o = this.arrData, y = o.length, x = this.chart.gridArea.width, t = this.chart.gridArea.height, i = this.realXAxis, j = this.realYAxis, v = 2 * (i.actualVisibleMaximum - i.actualVisibleMinimum) / x, w = 2 * (j.actualVisibleMaximum - j.actualVisibleMinimum) / t, a = [], r, s, c, b, p = 0, q = 0, k = 0, l = 0, m = false, d, f, g, e = [], h = 0;
        h < y;
        h++) {
        d = o[h];
        if (d === null) {
        ...
        a.push(s);
        e.push({
            dataItem: d, index: h, x: c, y: b;
        }
        );
    } var n = this._createShape(a, u);
    if (n) n.context = {
        chart: this.chart, series: this, points: e;
    }
};
...
fb.prototype._createShape = function (i, f) {
    ...
    var g = this.chart.gridArea, k = g.y, j = g.y + g.height, e = this.realYAxis.getCrossingPosition();
...
s.prototype._renderCatData = function (p) {
    var r = this.arrData, i = this.chart.gridArea, z = i.x, E = z + i.width, e = this.realYAxis.getCrossingPosition();
    e = b.fitInRange(e, z, E);
    e = Math.round(e);
    var q = this.chart.series._findClusters(this, this.type), l = r.length, C = i.height / this.realXAxis.getZoom(), x = C / l, F = x / q.count, f = Math.round(this.pointWidth * F), B = q.count * f, n = (x - B) / 2;
    n = Math.round(n + q.index * f);
    f = Math.max(f, this.minimumWidth);
    for (var v = [], s = [], D = this.getLabelsOffset() + 2, c, k, d = 0;
        d < l;
        d++) {
        var m = r[d];
        if (m == null) continue;
        ...
        var u = {
            chart: this.chart, series: this, dataItem: r[d], index: d, x: this.realXAxis._getValue(d), y: h;
        }
            , j = new o(c, k, g, f);
        j.context = u;
        j.center = {
            x: Math.round(A), y: Math.round(k + f / 2);
        }
            ;
...
s.prototype._renderLinearData = function (m) {
    var n = this.arrData, g = this.chart.gridArea, x = g.x, E = x + g.width, d = this.realYAxis.getCrossingPosition();
    d = b.fitInRange(d, x, E);
    d = Math.round(d);
    var p = n.length, A = g.height / this.realXAxis.getZoom(), D = this._getSeriesFromThisType(), C = this._calcColumnScale(D) * A, l = this.pointWidth * C;
    l = Math.max(l, this.minimumWidth);
    for (var u = [], r = [], B = this.getLabelsOffset(), c, q, i, j, k, e = 0;
        e < p;
        e++) {
        k = n[e];
        ...
        var t = {
            chart: this.chart, series: this, dataItem: n[e], index: e, x: i, y: j;
        }
            ;
        ...
        var y = q - l / 2, h = new o(c, y, f, l);
        h.context = t;
        h.center = {
            x: Math.round(z), y: Math.round(y + l / 2);
        }
            ;
...
s.prototype._addAnimation = function (c, g, f) {
    ...
    var d = a.duration / f, e = a.delayTime + g * d;
    a.delayTime = e;
    a.duration = d;
    this.chart.storyboard.addAnimation(a);
}
...
s.prototype._setIsAnimReversed = function (c) {
    var a = c.context.y < this.realYAxis.getCrossing(), b = this.realYAxis.reversed;
...
s.prototype._getPixelMargins = function (a) {
    if (a.isVertical() == false) {
        var b = f.prototype._getPixelMargins.call(this, a), c = a.length / 10, k = Math.max(c, b.left), j = Math.max(c, b.right);
        return {
            left: k, right: j;
        }
        ...
        ;
    var h = 4, i = this._getSeriesFromThisType(), e = this._calcColumnScale(i), g = a.length, d = .5 * e * g + h;
    return {
        left: d, right: d;
    }
};
...
s.prototype._getDataPointLabel = function (l, g, m, f, h) {
    var i = l <= this.realYAxis.getCrossing(), j = this._getLabelText(h), b = new k(j);
    d.setShadows(b, this, this.chart);
    a.extend(b, this.labels);
    b.measure(this.chart.ctx);
    b.y = m;
...
db.prototype._processData = function () {
    this.arrData = null;
    if (this.data) if (!this.xValuesType && !this.yValuesType && !this.sizeValuesType) this.arrData = b.cloneArray(this.data);
    else {
        this.arrData = [];
        for (var d = 0;
            d < this.data.length;
        ...
        var c = this.chart.arrDataSource;
        if (c) {
            var i = this.xValuesField, j = this.yValuesField, h = this.sizeValuesField, f = b.processDataField(c, i), g = b.processDataField(c, j), e = b.processDataField(c, h);
...
db.prototype._initData = function () {
    ...
    if (a.isArray(c) == false) return;
    for (var k = i, j = l, h = i, g = l, e = i, d = l, q = c.length, b = 0;
        b < q;
        b++) {
...
db.prototype._render = function (m) {
    if (!this.hasRealData || !this.isInScene()) return;
    var h = this.arrData, o = this.chart, p = o.options, g = p.maxBubbleSize;
    if (!g) {
        var r = o.gridArea;
    ...
    if (!f) f = this.maxSize;
    for (var i = h.length, j, k, e, c, b, a = 0;
        a < i;
        a++) {
        var d = h[a];
        if (d == null) continue;
        ...
        if (this.labels && this.labels.visible !== false) {
            var s = this._getLabelValue(b, a), n = this._getDataPointLabel(b, j, k, q / 2, s);
            m.push(n);
...
K.prototype._renderCatData = function (p) {
    var r = this.arrData, j = this.chart.gridArea, C = j.y, B = j.y + j.height, d = this.realYAxis.getCrossingPosition();
    d = b.fitInRange(d, C, B);
    d = Math.round(d);
    var q = this.chart.series._findClusters(this, this.type), k = r.length, y = j.width / this.realXAxis.getZoom(), v = y / k, A = v / q.count, f = Math.round(this.pointWidth * A), x = q.count * f, n = (v - x) / 2;
    n = Math.round(n + q.index * f);
    f = Math.max(f, this.minimumWidth);
    for (var u = [], s = [], z = this.getLabelsOffset(), g, h, e, c = 0;
        c < k;
        c++) {
        var l = r[c];
        if (l == null) continue;
        ...
        var t = {
            chart: this.chart, series: this, dataItem: r[c], index: c, x: this.realXAxis._getValue(c), y: e;
        }
            , i = new o(g, h, f, m);
        i.context = t;
        i.center = {
            x: Math.round(g + f / 2), y: Math.round(w);
        }
            ;
...
K.prototype._renderLinearData = function (m) {
    var n = this.arrData, j = this.chart.gridArea, B = j.y, z = j.y + j.height, c = this.realYAxis.getCrossingPosition();
    c = b.fitInRange(c, B, z);
    c = Math.round(c);
    var p = n.length, w = j.width / this.realXAxis.getZoom(), A = this._getSeriesFromThisType(), y = this._calcColumnScale(A) * w, i = this.pointWidth * y;
    i = Math.max(i, this.minimumWidth);
    for (var s = [], q = [], x = this.getLabelsOffset(), e, f, u, g, d = 0;
        d < p;
        d++) {
        var k = n[d];
        if (k == null || a.isArray(k) == false) continue;
        ...
        var r = {
            chart: this.chart, series: this, dataItem: n[d], index: d, x: e, y: g;
        }
            ;
        ...
        var t = e - i / 2, h = new o(t, f, i, l);
        h.context = r;
        h.center = {
            x: Math.round(t + i / 2), y: Math.round(v);
        }
            ;
...
x.prototype._processData = function () {
    ...
    if (this.data) {
        ...
        if (a) {
            var d = this.dataLabelsField, e = this.dataValuesField, f = b.processDataField(a, d), g = b.processDataField(a, e);
            this.arrData = b.mergeArraysXY(f, g);
            this.expIndexes = [];
            for (var c = 0;
                c < a.length;
                c++) {
                var h = a[c], i = h[this.explodedField];
...
x.prototype._getYValues = function () {
    for (var e = [], f = this.arrData, g = f.length, d = 0;
        d < g;
        d++) {
        var b = f[d];
        if (b == null) continue;
...
x.prototype._getLegendItems = function (c) {
    ...
    var i = this.arrData;
    if (!i) return d;
    for (var m = this._getYValues(), h, o = i.length, b = 0;
        b < o;
        b++) {
        var e = i[b];
        if (e == null) continue;
        var k = a.inArray(b, this.hiddenSlices) == -1, p;
        ...
        var l = {
            chart: this.chart, series: this, dataItem: e, index: b, value: m[b];
        }
            , f = a.extend(false, {
            }
                , c, {
                    context: l, text: h, marker: g;
                }
            );
        if (!k) {
...
x.prototype._hideFromLegend = function (e) {
    var d = e.index, b = this.hiddenSlices, c = a.inArray(d, b);
    if (c > -1) b.splice(c, 1);
    else b.push(d);
}
    ;
x.prototype.getTotal = function () {
    for (var c = this._getYValues(), d = 0, b = 0;
        b < c.length;
        b++) {
...
x.prototype._getTooltip = function (c) {
    var a = "<b>" + c.value + "</b><br/>", d = c.dataItem[0];
...
function p(b) {
    var c = a.extend(true, {
        ...
        , this.defaults, {
            startAngle: -90, explodedRadius: 10, allowExplodeSlices: true, explodedSlices: [], labelsPosition: "inside", labelsAlign: "circle", labelsExtend: 20, leaderLineWidth: 1, leaderLineStrokeStyle: "black", innerExtent: 0, outerExtent: 1;
        }
    );
    this.defaults = c;
    x.call(this, b);
...
p.prototype._createLabels = function () {
    var D = this.arrData, f = {
    }
        ;
    if (!D) return f;
    var n = this.chart.gridArea, M = n.width, L = n.height, C = this._getYValues(), F = C.length, G = this.getTotal(), r = this.margin, m = this._calcRadius({
        h: r, v: r;
    }
    );
    if (m < 0) return f;
    var w = m * this.innerExtent, j = m * this.outerExtent, s = n.x + M / 2, t = n.y + L / 2, K = Math.PI * 2 / G, p = b.radians(this.startAngle), u = this.explodedRadius;
    if (!this.labels || this.labels.visible === false) return f;
    for (var l, A, q = [], p = b.radians(this.startAngle), B = this.labelsPosition == "outside", v = this.hiddenSlices, H = F - v.length, o = 0, d = 0;
        d < F;
        d++) {
        if (a.inArray(d, v) != -1) {
            o++;
        ...
        var i = C[d], y = 100 * i / G, x = i, z = p + i * K;
        switch (this.labels.valueType) {
            case "percentage": x = y;
        }
        if (i == 0) continue;
        var J = this._getLabelText(x), e = (p + z) / 2, c = new k(J);
        c.textBaseline = "top";
        a.extend(c, this.labels);
        c.context = {
            chart: this.chart, series: this, dataItem: D[d], index: d, value: i, percentage: y;
        }
            ;
        this.chart.elem.trigger("dataPointLabelCreating", c);
        var E = c.measure(this.chart.ctx), g = s, h = t;
        A = this.isExploded(d);
        if (A) {
            h = t + u * Math.sin(e);
            g = s + u * Math.cos(e);
        }
        if (B) l = this._getSliceCenter(g, h, e, j + this.labelsExtend);
        else {
            var I = w ? (w + j) / 2 : j * .6;
        ...
        q.push(c);
        segment = {
            pt: this._getSliceCenter(g, h, e, j), angle: e, index: d, animIndex: d - o, cx: g, cy: h;
        }
            ;
    ...
    a.extend(f, {
        x: s, y: t, radius: m, margin: r, labels: q;
    }
    );
    return f;
}
...
p.prototype._arrangeLabels = function (c) {
    ...
    if (!j || this.labelsPosition != "outside") return {
        margins: {
            h: this.margin, v: this.margin;
        }
        , lines: [];
    };
    var f = [], e = [], u = this._calculateLabelsMargin(j, c), v = this._calcRadius(u), w = v * this.innerExtent, m = v * this.outerExtent;
    c.radius = m;
    for (var a, t, p = l, o = l, b = 0;
        b < j.length;
        b++) {
        a = j[b];
        t = a.width;
    ...
    var s = [], d, g, k, q, h, i, n = this.explodedRadius;
    for (b = 0;
        b < f.length;
        b++) {
        a = f[b];
...
p.prototype.labelComparator = function (a) {
    a = a ? -1 : 1;
    return function (e, f) {
        var c = e.segment.angle, d = f.segment.angle, c = b.normalizeAngle(c + 3 * Math.PI / 2), d = b.normalizeAngle(d + 3 * Math.PI / 2);
        return (c - d) * a;
    }
};
p.prototype._addLabelLine = function (m, l, t) {
    var j = 4, p = 4, o = this.labelsAlign == "circle", d = m.segment, a = [], i = l.radius;
    if (this.isExploded(d.index)) i += this.explodedRadius;
    var k = l.x, s = l.y;
    a.push(d.pt.x);
    a.push(d.pt.y);
    var h = m.x, f = m.y + m.height / 2, c, e, g, r = new Z(k - i, s - i, i);
    if (d.isLeft) {
        h += m.width + p;
        e = h + j;
        c = b.intersection({
...
p.prototype._renderSlices = function (C, B) {
    var p = this.arrData;
    if (!p) return;
    var f = this.chart.gridArea, F = f.width, E = f.height, n = this._getYValues(), q = n.length, r = this.getTotal(), h = this._calcRadius(B);
    if (h < 0) return;
    for (var w = h * this.innerExtent, x = h * this.outerExtent, u = f.x + F / 2, v = f.y + E / 2, D = Math.PI * 2 / r, e = b.radians(this.startAngle), G = this.fillStyles, j = this.explodedRadius, l = this.hiddenSlices, z = q - l.length, k = 0, m, c = 0;
        c < q;
        c++) {
        if (a.inArray(c, l) != -1) {
            k++;
        ...
        var i = n[c], y = 100 * i / r, g = e + i * D, s = u, t = v;
        m = this.isExploded(c);
        if (m) {
            var o = (e + g) / 2;
            t = v + j * Math.sin(o);
        ...
        var d = new yb(s, t, w, x, e, g), A = {
            chart: this.chart, series: this, dataItem: p[c], index: c, value: i, percentage: y;
        }
            ;
        d.context = A;
...
p.prototype._distancesBetweenLabels = function (b) {
    ...
    var c = [], d, a = b[0], f = this.chart.gridArea, h = f.height, g = f.y;
    c.push(a.y - g);
    for (var e = 1;
        e < b.length;
        e++) {
...
p.prototype._hLabelAlign = function (a, j, i, d) {
    var e = a.segment, k = this.labelsAlign == "circle";
    if (k) {
        var g = d.x, h = d.y, b = d.radius + this.labelsExtend;
        if (this.isExploded(e.index)) b += this.explodedRadius;
        var c = Math.min(Math.abs(h - a.y), Math.abs(h - a.y - a.height));
        if (c < b) if (e.isLeft) a.x = g - a.width - Math.sqrt(b * b - c * c);
...
p.prototype._reflowLabels = function (d, e) {
    var g = e.length, b, f = this.chart.gridArea, h = f.height, c = f.y;
    d[0] += 2;
    for (var a = 0;
        a < g;
        a++) {
...
p.prototype._calculateLabelsMargin = function (e, g) {
    if (this.labelsPosition != "outside") return {
        ...
        ;
    for (var a = g.margin, b = this.margin, d = this.labelsExtend, c = 0;
        c < e.length;
        c++) {
        var f = e[c];
...
p.prototype._render = function (b) {
    if (!this.hasRealData || !this.isInScene()) return;
    var c = this._createLabels(b), d = this._arrangeLabels(c);
...
p.prototype._calcRadius = function (b) {
    var f = b.v || this.margin, c = this.chart.gridArea, e = c.width - 2 * b.h, d = c.height - 2 * f, a;
    if (e < d) a = e / 2;
    else a = d / 2;
...
p.prototype._getSliceCenter = function (c, d, b, a) {
    return {
        x: c + a * Math.cos(b), y: d + a * Math.sin(b);
    }
};
p.prototype._getTooltip = function (a) {
    var e = this.chart.stringFormat(a.percentage, "%.2f%%"), c = "<b>" + a.value + " (" + e + ")</b><br/>", d = a.dataItem[0];
...
p.prototype._addSliceAnimation = function (c, g, f) {
    var b = this._getAnimation();
    if (!b || b.enabled === false) return;
    var a = new xb(b, c, "endAngle", c.startAngle, c.endAngle), d = a.duration / f, e = a.delayTime + g * d;
    a.delayTime = e;
    a.duration = d;
    this.chart.storyboard.addAnimation(a);
}
...
p.prototype._handleEvent = function (j, i) {
    ...
    var h = i.context, f = h.index, e = this, c = e.expIndexes, b = e.chart;
    switch (j) {
        case "MouseDown": case "TouchEnd": var d = a.inArray(f, c);
            if (d == -1) c.push(f);
            else c.splice(d, 1);
...
function Sb(b) {
    var c = a.extend(true, {
        ...
        , this.defaults, {
            lineWidth: 0, markers: null, stepDirection: "forward"
        }
    );
    this.defaults = c;
    B.call(this, b);
...
Sb.prototype._createShape = function (e, l) {
    for (var i, g, f, h, c, a = [], j = e.length, k = this.stepDirection == "forward", b = 0;
        b < j;
        b += 2) {
        i = e[b];
        g = e[b + 1];
...
Rb.prototype._createShape = function (g, l) {
    ...
    var i = this.chart.gridArea, p = i.y, o = i.y + i.height, h = this.realYAxis.getCrossingPosition();
...
v.prototype._render = function (t) {
    if (!this.hasRealData || !this.isInScene()) return;
    for (var i = this.arrData, A = this.chart.series._findStackedClusters(this, this.type), m = this.chart.series._getStackedSeriesFromType(this.type, this.stackedGroupName), v = i.length, n = this.markers != null && this.markers.isVisible(), j = [], r = [], s = [], p = [], x = this.getLabelsOffset(), y = this.xAxisType, k, l, b, g, d, f, c = 0;
        c < v;
        c++) {
        b = i[c];
        if (b == null) continue;
        ...
        j.push(k);
        j.push(l);
        var e = b.actualValue, u = this._getStackedTotal(m, h), w = e > 0 ? u.positive : u.negative, o = w != 0 ? 100 * Math.abs(e) / Math.abs(w) : 0, z = {
            chart: this.chart, series: this, dataItem: b, index: c, x: f, y: d, value: e, percentage: o;
        }
            ;
        !n && r.push({
            dataItem: b, index: c, x: f, y: d, value: e, percentage: o;
        }
        );
        if (this.realYAxis.isValueVisible(d) === false) continue;
    ...
    if (!n && q) q.context = {
        chart: this.chart, series: this, points: r;
    }
        ;
    else a.merge(t, s);
    return p;
...
qb.prototype._getTooltip = function (b) {
    var d = this.chart.stringFormat(b.percentage, "%.2f%%"), a = "<b>" + b.value + " (" + d + ")</b><br/>";
    if (this.title) {
...
function T(b) {
    var c = a.extend(true, {
        ...
        , this.defaults, {
            lineWidth: 0, markers: null, stackedGroupName: ""
        }
    );
    this.defaults = c;
    f.call(this, b);
...
T.prototype._render = function (v) {
    if (!this.hasRealData || !this.isInScene()) return;
    for (var i = this.arrData, o = this.chart.series._findStackedClusters(this, this.type), f = this.chart.series._getStackedSeriesFromType(this.type, this.stackedGroupName), u = this.xAxisType, w = i.length, j = [], k = [], n = [], l, r, s, a, e, d, x, h, b = 0;
        b < w;
        b++) {
        a = i[b];
        if (a == null) continue;
        ...
        var g = a.actualValue, p = this._getStackedTotal(f, c), q = g > 0 ? p.positive : p.negative, t = q != 0 ? 100 * Math.abs(g) / Math.abs(q) : 0;
        n.push({
            dataItem: a, index: b, x: h, y: d, value: g, percentage: t;
        }
        );
    } var m = this._createShape(j, k, v);
    if (m) m.context = {
        chart: this.chart, series: this, points: n;
    }
};
...
T.prototype._createShape = function (c, d, b) {
    if (!d.length) return fb.prototype._createShape.call(this, c, b);
...
pb.prototype._createShape = function (c, d, b) {
    if (!d.length) return Bb.prototype._createShape.call(this, c, b);
...
L.prototype._renderCatData = function (E) {
    var s = this.arrData;
    if (!s) return;
    var r = this.chart.gridArea, O = r.y, N = r.y + r.height, i = this.realYAxis.getCrossingPosition();
    i = b.fitInRange(i, O, N);
    i = Math.round(i);
    var d = this.chart.series._findStackedClusters(this, this.type), G = s.length, J = r.width / this.realXAxis.getZoom(), C = J / G, M = C / d.groupCount, g = Math.round(this.pointWidth * M), I = d.groupCount * g, v = (C - I) / 2;
    v = Math.round(v + d.groupIndex * g);
    g = Math.max(g, this.minimumWidth);
    for (var B = [], A = [], t = this.chart.series._getStackedSeriesFromType(this.type, this.stackedGroupName), K = this.getLabelsOffset(), f, h, e, c = 0;
        c < G;
        c++) {
        e = s[c];
        if (e == null) continue;
        ...
        var n = this._getPrevStackedPosition(t, d.index, q, i, this.realYAxis, l >= 0), w = h, p;
        if (h <= n) p = n - h;
        else {
            p = h - n;
            h = n;
        }
        var m = e.actualValue, F = this._getStackedTotal(t, q), H = m > 0 ? F.positive : F.negative, y = H != 0 ? 100 * Math.abs(m) / Math.abs(H) : 0, u = {
            chart: this.chart, series: this, dataItem: s[c], index: c, value: m, x: this.realXAxis._getValue(c), y: l, percentage: y;
        }
            , k = new o(f, h, g, p);
        k.context = u;
        k.center = {
            x: Math.round(f + g / 2), y: Math.round(w);
        }
            ;
        ...
        if (this.labels && this.labels.visible !== false && e.actualValue != 0 && this.realXAxis.isValueVisible(c + .5)) {
            ...
            var z = this.labels.position == "outside", L = z ? K : -p / 2, j = this._getDataPointLabel(e.actualValue, f, w, L, x);
...
L.prototype._renderLinearData = function (C) {
    var h = this.arrData;
    if (!h) return;
    var s = this.chart.gridArea, M = s.y, J = s.y + s.height, i = this.realYAxis.getCrossingPosition();
    i = b.fitInRange(i, M, J);
    i = Math.round(i);
    var e = this.chart.series._findStackedClusters(this, this.type), m = this.chart.series._getStackedSeriesFromType(this.type, this.stackedGroupName), L = h.length, F = s.width / this.realXAxis.getZoom(), I = this._calcColumnScale(m) * F, t = this.pointWidth * I;
    t = Math.max(t, this.minimumWidth);
    for (var A = [], z = [], G = this.getLabelsOffset(), f, g, c, d = 0;
        d < L;
        d++) {
        c = h[d];
        if (c == null) continue;
        ...
        var p = this._getPrevStackedPosition(m, e.index, r, i, this.realYAxis, l >= 0), v = g, q;
        if (g <= p) q = p - g;
        else {
            q = g - p;
            g = p;
        }
        var n = c.actualValue, D = this._getStackedTotal(m, r), E = n > 0 ? D.positive : D.negative, x = E != 0 ? 100 * Math.abs(n) / Math.abs(E) : 0, u = {
            chart: this.chart, series: this, dataItem: h[d], index: d, value: n, x: h[d][0], y: l, percentage: x;
        }
            , K = f - t / 2, k = new o(K, g, t, q);
        k.context = u;
        k.center = {
            x: Math.round(f), y: Math.round(v);
        }
            ;
        ...
        if (this.labels && this.labels.visible !== false && c.actualValue != 0) {
            ...
            var y = this.labels.position == "outside", H = y ? G : -q / 2, j = this._getDataPointLabel(c.actualValue, f, v, H, w);
...
I.prototype._renderCatData = function (E) {
    var r = this.arrData;
    if (!r) return;
    var v = this.chart.gridArea, G = v.x, N = G + v.width, j = this.realYAxis.getCrossingPosition();
    j = b.fitInRange(j, G, N);
    j = Math.round(j);
    var d = this.chart.series._findStackedClusters(this, this.type), H = r.length, K = v.height / this.realXAxis.getZoom(), C = K / H, O = C / d.groupCount, g = Math.round(this.pointWidth * O), J = d.groupCount * g, u = (C - J) / 2;
    u = Math.round(u + d.groupIndex * g);
    g = Math.max(g, this.minimumWidth);
    for (var B = [], A = [], s = this.chart.series._getStackedSeriesFromType(this.type, this.stackedGroupName), L = this.getLabelsOffset() + 2, f, i, e, c = 0;
        c < H;
        c++) {
        e = r[c];
        if (e == null) continue;
        ...
        var n = this._getPrevStackedPosition(s, d.index, p, j, this.realYAxis, l >= 0), w = f, q;
        if (f <= n) q = n - f;
        else {
            q = f - n;
            f = n;
        }
        var m = e.actualValue, F = this._getStackedTotal(s, p), I = m > 0 ? F.positive : F.negative, y = I != 0 ? 100 * Math.abs(m) / Math.abs(I) : 0, t = {
            chart: this.chart, series: this, dataItem: r[c], index: c, value: m, x: this.realXAxis._getValue(c), y: l, percentage: y;
        }
            , k = new o(f, i, q, g);
        k.context = t;
        k.center = {
            x: Math.round(w), y: Math.round(i + g / 2);
        }
            ;
        this._setShapeSettings(k, c);
        E.push(k);
        ...
        if (this.labels && this.labels.visible !== false && e.actualValue != 0 && this.realXAxis.isValueVisible(c + .5)) {
            ...
            var z = this.labels.position == "outside", M = z ? L : -q / 2, h = this._getDataPointLabel(e.actualValue, w, i, M, x);
            if (!z) {
...
I.prototype._renderLinearData = function (C) {
    var h = this.arrData;
    if (!h) return;
    var u = this.chart.gridArea, E = u.x, L = E + u.width, i = this.realYAxis.getCrossingPosition();
    i = b.fitInRange(i, E, L);
    i = Math.round(i);
    var s = this.chart.series._getStackedSeriesFromType(this.type, this.stackedGroupName), f = this.chart.series._findStackedClusters(this, this.type), N = h.length, G = u.height / this.realXAxis.getZoom(), K = this._getSeriesFromThisType(), J = this._calcColumnScale(K) * G, r = this.pointWidth * J;
    r = Math.max(r, this.minimumWidth);
    for (var A = [], z = [], H = this.getLabelsOffset() + 2, e, l, c, d = 0;
        d < N;
        d++) {
        c = h[d];
        if (c == null) continue;
        ...
        var n = this._getPrevStackedPosition(s, f.index, p, i, this.realYAxis, k >= 0), v = e, q;
        if (e <= n) q = n - e;
        else {
            q = e - n;
            e = n;
        }
        var m = c.actualValue, D = this._getStackedTotal(s, p), F = m > 0 ? D.positive : D.negative, x = F != 0 ? 100 * Math.abs(m) / Math.abs(F) : 0, t = {
            chart: this.chart, series: this, dataItem: h[d], index: d, value: m, x: h[d][0], y: k, percentage: x;
        }
            , M = l - r / 2, j = new o(e, M, q, r);
        j.context = t;
        j.center = {
            x: Math.round(v), y: Math.round(l);
        }
            ;
        this._setShapeSettings(j, d);
        C.push(j);
        ...
        if (this.labels && this.labels.visible !== false && c.actualValue != 0) {
            ...
            var y = this.labels.position == "outside", I = y ? H : -q / 2, g = this._getDataPointLabel(c.actualValue, v, l, I, w);
            if (!y) {
...
gb.prototype._getPixelMargins = function (b) {
    if (b.isVertical() || !this.hasData()) return {
        ...
        ;
    var e = 4, f = this._getSeriesFromThisType(), c = this._calcColumnScale(f), d = b.length, a = .5 * c * d + e;
    return {
        left: a, right: a;
    }
};
...
H.prototype._renderCatData = function (z) {
    var h = this.arrData, A = this.chart.gridArea, v = this.categories, k = v.length, x = A.width / this.realXAxis.getZoom(), t = x / k, f = Math.round(this.pointWidth * t), y = Math.round((t - f) / 2);
    f = Math.max(f, this.minimumWidth);
    for (var q = [], n, l, m, d, i, j, b = 0;
        b < h.length;
        b++) {
        var e = h[b];
        if (e == null) continue;
        ...
        var s = {
            chart: this.chart, series: this, dataItem: h[b], index: b, catIndex: d, x: this.realXAxis._getValue(d), from: i, to: j;
        }
            , w = Math.min(l, m), u = Math.abs(l - m), g = new o(n, w, f, u);
        g.context = s;
...
H.prototype._renderLinearData = function (x) {
    var g = this.arrData, y = this.chart.gridArea, j = g.length, u = y.width / this.realXAxis.getZoom(), w = this._getSeriesFromThisType(), v = this._calcColumnScale(w) * u, f = this.pointWidth * v;
    f = Math.max(f, this.minimumWidth);
    for (var q = [], n, l, m, k, h, i, b = 0;
        b < j;
        b++) {
        var d = g[b];
        if (d == null || a.isArray(d) == false) continue;
        ...
        i = d[2];
        var r = {
            chart: this.chart, series: this, dataItem: g[b], index: b, catIndex: b, x: k, from: h, to: i;
        }
            ;
        n = this.realXAxis.getPosition(k);
        l = Math.round(this.realYAxis.getPosition(h));
        m = Math.round(this.realYAxis.getPosition(i));
        var z = n - f / 2, t = Math.min(l, m), s = Math.abs(l - m), e = new o(z, t, f, s);
        e.context = r;
...
H.prototype._setIsAnimReversed = function (c) {
    var a = c.context, b = this.realYAxis.reversed;
...
t.prototype._renderCatData = function (A) {
    var k = this.arrData, g = this.chart.gridArea, w = this.categories, n = w.length, y = g.height / this.realXAxis.getZoom(), u = y / n, d = this.pointWidth * u, z = Math.round((u - d) / 2);
    d = Math.max(d, this.minimumWidth);
    for (var r = [], i, j, p, e, l, m, b = 0;
        b < k.length;
        b++) {
        var f = k[b];
        if (f == null) continue;
        ...
        var t = {
            chart: this.chart, series: this, dataItem: k[b], index: b, catIndex: e, x: this.realXAxis._getValue(e), from: l, to: m;
        }
            , x = Math.min(i, j), v = Math.abs(i - j), h = new o(x, p, v, d);
        h.context = t;
...
t.prototype._renderLinearData = function (y) {
    var j = this.arrData, e = this.chart.gridArea, m = j.length, v = e.height / this.realXAxis.getZoom(), x = this._getSeriesFromThisType(), w = this._calcColumnScale(x) * v, i = this.pointWidth * w;
    i = Math.max(i, this.minimumWidth);
    for (var r = [], g, h, p, n, k, l, b = 0;
        b < m;
        b++) {
        var d = j[b];
        if (d == null || a.isArray(d) == false) continue;
        ...
        l = d[2];
        var s = {
            chart: this.chart, series: this, dataItem: j[b], index: b, catIndex: b, x: n, from: k, to: l;
        }
            ;
        ...
        var z = p - i / 2, u = Math.min(g, h), t = Math.abs(g - h), f = new o(u, z, t, i);
        f.context = s;
...
W.prototype._getTooltip = function (b) {
    var d = this.realYAxis, a = d._getTooltip(b.from).replace("<br/>", "") + " - " + d._getTooltip(b.to);
    if (this.title) {
        var e = c.getColorFromFillStyle(this.fillStyle);
...
G.prototype._processData = function () {
    this.arrData = null;
    if (this.data) if (!this.xValuesType && !this.highValuesType && !this.lowValuesType && !this.openValuesType && !this.closeValuesType) this.arrData = b.cloneArray(this.data);
    else {
        this.arrData = [];
        for (var d = 0;
            d < this.data.length;
        ...
        var c = this.chart.arrDataSource;
        if (c) {
            var n = this.xValuesField, k = this.highValuesField, m = this.lowValuesField, l = this.openValuesField, j = this.closeValuesField, i = b.processDataField(c, n), f = b.processDataField(c, k), h = b.processDataField(c, m), g = b.processDataField(c, l), e = b.processDataField(c, j);
...
G.prototype._render = function (q) {
    if (!this.hasRealData || !this.isInScene()) return;
    var n = this.arrData, r = this.chart.gridArea, j = n.length, d, g, i, h, e, l = r.width / this.realXAxis.getZoom(), f;
    ...
    for (var c = 0;
        ...
        var m = c;
        switch (this.xAxisType) {
            case "LinearAxis": case "DateTimeAxis": d = b[0];
                m = d;
                break;
            case "CategoryAxis": d = c + .5;
        }
        g = b[1];
        i = b[2];
        h = b[3];
        e = b[4];
        var p = {
            chart: this.chart, series: this, dataItem: b, index: c, x: this.realXAxis._getValue(m), high: g, low: i, open: h, close: e;
        }
            ;
...
G.prototype._getPixelMargins = function (a) {
    ...
    if (!this.hasData()) return {
        ...
        ;
    var e = 4, c = this._calcColumnScale([this]), d = a.length, b = .5 * c * d + e;
    return {
        left: b, right: b;
    }
};
...
J.prototype._render = function (l) {
    if (!this.hasRealData || !this.isInScene()) return;
    for (var o = this.arrData, p = o.length, i = this.markers != null && this.markers.isVisible(), v = this.labels != null && this.labels.visible !== false, c = [], j = [], n = [], m = [], s = this.getLabelsOffset(), g, h, e, u = this.realYAxis.cx, q = this.realYAxis.cy, d = 0;
        d < p;
        d++) {
        var f = o[d];
        if (f === null) {
        ...
        c.push(h);
        !i && j.push({
            dataItem: f, index: d, x: this.realXAxis._getValue(d), y: e;
        }
        );
    ...
    if (k) {
        l.push(k);
        if (!i) k.context = {
            chart: this.chart, series: this, points: j;
        }
    } a.merge(l, n);
    a.merge(l, m);
}
...
E.prototype._render = function (m) {
    if (!this.hasRealData || !this.isInScene()) return;
    for (var n = this.arrData, q = n.length, i = this.markers != null && this.markers.isVisible(), w = this.labels != null && this.labels.visible !== false, c = [], j = [], p = [], o = [], t = this.getLabelsOffset(), g, h, e, f, v = this.realYAxis.cx, r = this.realYAxis.cy, d = 0;
        d < q;
        d++) {
        var k = n[d];
        if (k === null) {
        ...
        c.push(h);
        var x = {
            chart: this.chart, series: this, dataItem: n[d], index: d, x: e, y: f;
        }
            ;
        !i && j.push({
            dataItem: k, index: d, x: e, y: f;
        }
        );
...
function U(b) {
    var c = a.extend(true, {
        ...
        , this.defaults, {
            lineWidth: 2, trendlineType: "linear", markers: {
            }
        }
    );
    this.defaults = c;
...
U.prototype._initData = function () {
    for (var j = this.tData = this._getTrendlineResult(), f = i, e = l, d = i, c = l, k = j.length, h = 0;
        h < k;
        h++) {
        var g = j[h];
        if (g == null) continue;
...
U.prototype._getTrendlineResult = function () {
    for (var j = this.arrData, k = j.length, d, f, i = [], c = [], h = [], b = 0;
        b < k;
        b++) {
        var e = j[b];
        if (e == null) continue;
        if (a.isArray(e) == false) {
            ...
            var g = e[0];
            switch (this.xAxisType) {
                case "CategoryAxis": g = b + .5;
                    break;
                case "DateTimeAxis": g = g.getTime();
            }
            c.push(g);
            h.push(e[1]);
        }
    } switch (this.trendlineType) {
        case "exp": case "exponential": d = this._getExpRegression(c, h);
            for (var b = 0;
                b < c.length;
                b++) {
            ...
            break;
        case "linear": default: d = this._getLinearRegression(c, h);
            for (var b = 0;
                b < c.length;
                b++) {
                f = d[0] * c[b] + d[1];
...
U.prototype._getRegression = function (m, g) {
    var h = this.trendlineType, e = this.arrData.length, b = 0, f = 0, i = 0, j = 0, l = 0, d = [], c = [];
    if (h == "linear") {
        c = m;
        d = g;
    }
    else if (h == "exp" || h == "exponential") for (var a = 0;
        a < g.length;
        a++)if (g[a] <= 0) e--;
        else {
            c.push(m[a]);
...
U.prototype._getExpRegression = function (d, e) {
    var a = this._getRegression(d, e), c = Math.exp(a[0]), b = Math.exp(a[1]);
    return [c, b];
}
    ;
U.prototype._render = function (k) {
    if (!this.hasRealData || !this.isInScene()) return;
    var l = this.arrData, c = this.tData, b = [], g = 1;
    ...
    var a;
    switch (this.trendlineType) {
        case "exp": case "exponential": a = new u(b);
            break;
        case "linear": default: a = new q(b);
    }
    this._setShapeSettings(a);
    k.push(a);
    this._addLengthAnimation(a);
...
O.prototype._renderCatData = function (n) {
    for (var o = this.arrData, p = o.length, h = this.markers != null && this.markers.isVisible(), c = [], i = [], m = [], k = [], r = this.getLabelsOffset(), g, j, q, d, e = 0;
        e < p;
        e++) {
        var f = o[e];
        if (f === null) {
...
O.prototype._renderLinearData = function (o) {
    for (var p = this.arrData, q = p.length, g = this.markers != null && this.markers.isVisible(), s = this.labels != null && this.labels.visible !== false, c = [], h = [], n = [], l = [], r = this.getLabelsOffset(), j, k, e, d, f = 0;
        f < q;
        f++) {
        var i = p[f];
        if (i === null) {
...
jb.prototype._createShape = function (i, f) {
    ...
    var c = new q(h);
    this._setShapeSettings(c);
    var g = this.chart.gridArea, k = g.x, j = g.x + g.width, e = this.realYAxis.getCrossingPosition();
    e = b.fitInRange(e, k, j);
...
z.prototype._renderCatData = function (r) {
    for (var s = this.arrData, k = s.length, j = this.markers != null && this.markers.isVisible(), c = [], d = [], e = [], q = [], o = [], t = this.getLabelsOffset(), i, m, n, l, f, g, b = 0;
        b < k;
        b++) {
        var h = s[b];
        if (h === null) {
        ...
        d.push(n);
        if (!j) e[b] = e[2 * k - b - 1] = {
            dataItem: h, index: b, x: l, from: f, to: g;
        }
            ;
...
z.prototype._renderLinearData = function (r) {
    for (var s = this.arrData, k = s.length, h = this.markers != null && this.markers.isVisible(), u = this.labels != null && this.labels.visible !== false, i = [], j = [], d = [], q = [], o = [], t = this.getLabelsOffset(), g, m, n, l, e, f, b = 0;
        b < k;
        b++) {
        var c = s[b];
        if (c === null) {
        ...
        j.push(n);
        if (!h) d[b] = d[2 * k - b - 1] = {
            dataItem: c, index: b, x: l, from: e, to: f;
        }
            ;
    ...
    if (!h && p) p.context = {
        chart: this.chart, series: this, points: d;
    }
        ;
    else a.merge(r, q);
    return o;
...
z.prototype._addMarkersAndLabels = function (g, n, i, q, r, a, h, p, d, e, m) {
    var k = 0, o = this.arrData, s = p ? p : a;
    if (this.markers && this.markers.isVisible()) {
        var f = {
            chart: this.chart, series: this, dataItem: o[a], index: a, x: this.realXAxis._getValue(s), from: d, to: e;
        }
            ;
        k = this.markers.offset;
        var l = f.dataItem[3], b = this._addMarker(i, q, null, d, l, a, f);
        if (b.marker) {
        ...
        var j = this._getLabelValue(d, a), c = this._getDataPointLabel(d, i, q, m + k, j, d > e), f = {
            chart: this.chart, series: this, dataItem: o[a], index: a;
        }
            ;
        c.context = f;
        this.chart.elem.trigger("dataPointLabelCreating", c);
...
z.prototype._getDataPointLabel = function (j, i, e, c, f, g) {
    var h = this._getLabelText(f), b = new k(h);
    d.setShadows(b, this, this.chart);
    a.extend(b, this.labels);
    b.measure(this.chart.ctx);
    b.textAlign = "center";
...
function kb(b) {
    var c = a.extend(true, {
        ...
        , this.defaults, {
            lineWidth: 0, spacing: 0, inverted: false, dynamicSlope: false, dynamicHeight: false, neckRatio: .2;
        }
    );
    this.defaults = c;
    x.call(this, b);
...
kb.prototype._render = function (B) {
    ...
    for (var n = this.chart.gridArea, H = this.lineWidth, p = this.hiddenSlices, u = j.length, o = u - p.length, q = 0, r = this.neckRatio, z = this.inverted, m = this.margin, G = n.x + m, D = n.y + m, g = n.width - 2 * m, i = n.height - 2 * m, F = G + g / 2, J = D + i / 2, s = D, k, w = 0, t = this._getYValues(), h, e, d, f, E = g, l = 0, I, c, y, b = 0;
        b < u;
        b++) {
    ...
    for (b = 0;
        ...
        var x = {
            x: F - d / 2, y: s, w: d, h: f - e;
        }
            , A = {
                chart: this.chart, series: this, dataItem: j[b], index: b, value: c, percentage: y;
            }
...
kb.prototype._createSlice = function (f, g, h, j, a, q, o) {
    ...
    a.y += d;
    a.h -= 2 * d;
    var e = a.x + a.w / 2, s = a.y + a.h / 2, b = this.lineWidth / 2, p = e - f / 2 + b, n = e + f / 2 - b, m = e - g / 2 + b, l = e + g / 2 - b, k = h + b, i = j - b, r = [p, k, n, k, l, i, m, i], c = new R(r);
    c.context = o;
    this._setShapeSettings(c);
...
kb.prototype._createLabel = function (c, l, j, i, e) {
    ...
    var d = i;
    switch (this.labels.valueType) {
        case "percentage": d = e.percentage;
    }
    var g = c.x + c.w / 2, h = c.y + c.h / 2, f = this._getLabelText(d), b = new k(f);
    b.textAlign = "center";
    a.extend(b, this.labels);
    b.x = g;
    b.y = h;
...
kb.prototype._getNextValueIndex = function (c) {
    for (var f = this.arrData, d = this.hiddenSlices, e = f.length, b = c + 1;
        b < e;
        b++)if (a.inArray(b, d) == -1) return b;
    return c;
}
    ;
function ub(b) {
...
r.prototype.setOptions = function (d) {
    ...
    for (var f = [], g = this.userAxes, e = 0;
        ...
        if (g) {
            ...
            case "category": b = new F(a);
                break;
            case "dateTime": b = new j(a);
                break;
            case "linearRadius": b = new w(a);
                break;
            case "categoryAngle": b = new M(a);
                break;
            case "linearAngle": b = new D(a);
                break;
            case "linear": default: b = new n(a);
        }
...
r.prototype._initSeriesAxes = function () {
    ...
    for (var e = this.chart.series.items, d = 0;
        d < e.length;
        d++) {
        var c = e[d];
        if (!c.isInScene()) continue;
...
r.prototype._measure = function () {
    for (var c = this.items, a = false, b = 0;
        b < c.length;
        b++) {
...
m.prototype._initDefs = function () {
    ...
    var b = a.extend(true, {
        ...
        , this.defaults, {
            rangeSlider: {
                visible: true, breadth: 20;
            }
            , mouseWheelZoomStep: .001, mouseWheelScrollStep: .05, margin: 5, strokeStyle: "black", lineWidth: 1, zoomEnabled: false, visible: true;
        });
    this.defaults = b;
}
    ;
m.prototype._initSeries = function () {
    for (var c = new y(null, this.chart), d = this.chart.series.items, b = 0;
        b < d.length;
        b++) {
        var a = d[b];
...
m.prototype._setVisibleRanges = function () {
    ...
    this.jqRangeSlider.jqRangeSlider("update", {
        minimum: this.actualMinimum, maximum: this.actualMaximum, smallChange: a, largeChange: 2 * a, minRange: a / 100, range: {
            minimum: this.actualVisibleMinimum, maximum: this.actualVisibleMaximum;
        }
    });
};
m.prototype._arrange = function () {
    ...
    switch (this.location) {
        case "left": b = {
            left: this.x + this.width - g, top: this.y, width: c, height: this.height;
        }
            ;
            break;
        case "right": b = {
            left: this.x + this.lineWidth / 2, top: this.y, width: c, height: this.height;
        }
            ;
            break;
        case "top": b = {
            left: this.x, top: this.y + this.height - g, width: this.width, height: c;
        }
            ;
            break;
        case "bottom": b = {
            left: this.x, top: this.y + this.lineWidth / 2, width: this.width, height: c;
        }
    }if (b) {
        var h = this.isAxisVertical ? "vertical" : "horizontal";
        this.jqRangeSlider.css(b).jqRangeSlider("update", {
            orientation: h, reversed: this.reversed;
        }
        );
    }
};
m.prototype._moveVisibleRange = function (f, g) {
    var e = this.isAxisVertical, c = this.actualVisibleMinimum, b = this.actualVisibleMaximum, d = b - c, a = 0;
...
m.prototype._mouseWheelZoom = function (j) {
    var c = this.actualVisibleMinimum, b = this.actualVisibleMaximum, g = j * 100 * this.mouseWheelZoomStep * (this.actualVisibleMaximum - this.actualVisibleMinimum), d = this.chart.mouseInput;
    if (!d) return;
    var i = this.isAxisVertical ? d.locY : d.locX, a = (i - this.origin) / this.length, f, e;
    if (this.isAxisVertical) {
        f = 1 - a;
        e = a;
    }
...
m.prototype._mouseWheelScroll = function (d) {
    var c = this.actualVisibleMinimum, b = this.actualVisibleMaximum, a = this.mouseWheelScrollStep * d * (this.actualMaximum - this.actualMinimum);
...
F.prototype.getCategories = function () {
    ...
    var a = this.chart.arrDataSource;
    if (!a) return null;
    var c = this.categoriesField, d = b.processDataField(a, c);
    return d;
}
    ;
...
F.prototype._getLabelIntervals = function (d, c) {
    ...
    for (var f = [], h = Math.round(this.actualVisibleMinimum), g = this._getIntervalStart(h, d) + .5, a = g + e;
        a <= this.actualVisibleMaximum;
        a = b.round(a + d))f.push(a);
    return f;
}
    ;
...
n.prototype._initDefs = function () {
    ...
    var b = a.extend(true, {
        ...
        , this.defaults, {
            extendRangeToOrigin: false, logarithmic: false, logBase: 10, labels: {
...
n.prototype._initRange = function () {
    ...
    var h = Math.abs(d - e), u = this.DataType == "DateTimeAxis" ? 1e11 : 1e6;
    ...
    var o = 0, n = 0, f = j._getPixelMargins(this);
    if (this.isAxisVertical) {
    ...
    var a = e - o, c = d + n, t = this.series._isAnchoredToOrigin(), g = this.getCrossing();
    if (t && this.getOrientation() == "y") if (e >= g && a < g) a = g;
    else if (d <= g && c > g) c = g;
    if (this.extendRangeToOrigin) if (a > g) a = g;
    else if (c < g) c = g;
    if (this.logarithmic === true) {
        var s = 1;
        if (a < s) a = s;
...
n.prototype._getIntervals = function (c, a, h) {
    ...
    for (var f = [], g = this._getIntervalStart(this._getActualVisibleMinimum(), c), d = g + e;
        d <= this._getActualVisibleMaximum();
        d = b.round(d + c))f.push(Math.pow(this.logBase, d));
    return f;
}
    ;
n.prototype._getLogarithmicMinorIntervals = function (m, k) {
    for (var l = this._getMarkInterval(k.major, true), h = this._getIntervals(l, k.major, true), j = [], d = null, g = 0;
        g < h.length;
        g++) {
        var f = h[g];
        if (d == null) {
...
j.prototype._initDefs = function () {
    ...
    var b = a.jqChartDateFormat.masks, c = a.extend(true, {
    }
        , this.defaults, {
            labels: {
                yearsIntervalStringFormat: "yyyy", monthsIntervalStringFormat: b.shortDate, weeksIntervalStringFormat: b.shortDate, daysIntervalStringFormat: b.shortDate, hoursIntervalStringFormat: b.shortDate + " " + b.shortTime, minutesIntervalStringFormat: b.shortTime, secondsIntervalStringFormat: b.longTime, millisecondsIntervalStringFormat: b.longTime;
            }
            , skipEmptyDays: false;
        });
    this.defaults = c;
...
j.prototype._setMinMax = function (c, b) {
    if (this.minimum != null) if (a.type(this.minimum) == "date") this.actualMinimum = this.minimum.getTime();
...
j.prototype._calculateDateTimeInterval = function (j, i) {
    var h = i - j, f = .8 * this.maxInter200Px, g = Math.max(1, this.length), e = g / (200 * 10 / f), b = h / e;
    this.type = "year";
    var a = b / (1e3 * 60);
    if (a <= 1) {
        if (b <= 10) {
...
j.prototype._getIntervalStart = function (j, b, d) {
    if (d == null) return j;
    var a = new Date(j);
    if (b > 0 && b != 1) if (d == "months" && b <= 12 && b > 1) {
    ...
    switch (d) {
        case "years": var f = a.getFullYear() / b * b;
            if (f <= 0) f = 1;
...
j.prototype._initActualStringFormat = function () {
    ...
    switch (this.actualIntervalType) {
        case "years": this.actualStringFormat = this.labels.yearsIntervalStringFormat;
            break;
...
j.prototype._getIntervals = function (e, c) {
    if (this.customTickMarks) return this.customTickMarks;
    var g = [], h = this.actualVisibleMinimum, j = this.actualVisibleMaximum, a = this._getIntervalStart(h, e, this.actualIntervalType);
    while (a < h) a = this._incrementDateTime(a, e, this.actualIntervalType);
    if (c && c.intervalOffset) {
        var f = this.actualIntervalType, i = c.intervalOffset;
        if (c.intervalOffsetType) f = c.intervalOffsetType;
        a = this._incrementDateTime(a, i, f);
    }
    for (var b = a;
        b <= j;
        b = this._incrementDateTime(b, e, this.actualIntervalType)) {
...
j.prototype._getNextNonEmptyDay = function (b) {
    ...
    if (d == -1) return b;
    for (var f = g.addDays(new Date(b), 1), h = this.actualVisibleMaximum, c = f;
        c <= h;
        c = g.addDays(c, 1)) {
        var e = c.getTime();
...
j.prototype._addEmptyDaysOffset = function (c) {
    ...
    var a = c + this._getEmptyDaysOffset(c), d = this._getEmptyDaysOffset(a) - this._getEmptyDaysOffset(c);
    c = a;
    a += d;
    while (d) {
...
j.prototype._getEmptyDaysFromSeries = function (i) {
    ...
    for (var c = g.roundToDay(b[0][0]), d = 1;
        d < b.length;
        d++) {
        for (var h = g.roundToDay(b[d][0]), j = (h - c) / g.ticksInDay, e = 1;
            e < j;
            e++)f.push(g.addDays(c, e).getTime());
        c = h;
    }
    return f;
};
j.prototype._excludeDaysFromSeries = function (c, b) {
    a.each(c.arrData, function (f, d) {
        var e = g.roundToDay(d[0]).getTime(), c = a.inArray(e, b);
        c != -1 && b.splice(c, 1);
    }
    );
};
...
w.prototype._initDefs = function () {
    ...
    var b = a.extend(true, {
        ...
        , this.defaults, {
            innerExtent: .2, renderStyle: "circle", majorTickMarks: {
                visible: false;
            }
            , location: "radial", majorGridLines: {
                strokeStyle: "gray", lineWidth: 1, visible: true;
            }
        });
    this.defaults = b;
...
w.prototype._getLabels = function () {
    ...
    for (var m = this._getMaxOutsideTickMarksLength() + this.lblMargin, e = [], n = this._getMarkInterval(a, true), f = this._getLabelIntervals(n, a), g = f.length, j = a.showFirstLabel, k = a.showLastLabel, c = 0;
        c < g;
        c++) {
        if (!j && c == 0 || !k && c == g - 1) continue;
        var h = f[c], l = this.getLabel(h), d = this._createLabel(l, a), o = this.getPosition(h), q = this.cy, p = o, i = b.rotatePointAt(p, q, -Math.PI / 2, this.cx, this.cy);
        d.x = i.x - m;
        d.y = i.y;
        d.textAlign = "right";
        e.push(d);
...
w.prototype._getTickMarks = function (a, l) {
    ...
    for (var i = [], s = this._getMarkInterval(a, l), t = a.length, k = this._getIntervals(s, a, l), g = this.sharedAxis, f = g._getIntervals(g.actualInterval), p, r, q, c, d = 0;
        d < k.length;
        d++) {
        ...
        for (var e = 0;
            e < f.length;
            e++) {
            var v = f[e], j = this.sharedAxis._getAngle(v), n = b.rotatePointAt(p, r, j, this.cx, this.cy), o = b.rotatePointAt(q, c, j, this.cx, this.cy), m = new h(n.x, n.y, o.x, o.y);
...
w.prototype._getGridLines = function (a, g) {
    if (a == null || a.visible != true) return [];
    for (var d = [], h = this._getMarkInterval(a, g), e = this._getIntervals(h, a, true), c = 0;
        c < e.length;
        c++) {
...
w.prototype._render = function (d) {
    ...
    a.merge(c, e);
    return {
        postShapes: c, contextShapes: e;
    }
};
w.prototype._getRenderShape = function (b) {
    var d = this.getPosition(b), a = d - this.origin + this.extent, c = this._createRenderShape(this.cx - a, this.cy - a, a);
    return c;
}
    ;
w.prototype._createRenderShape = function (f, g, e) {
    ...
    if (this.renderStyle != "polygon") {
        if (b.isNull(a.startAngle) || b.isNull(a.endAngle) || a.startAngle - a.endAngle == 360) return new Z(f, g, e);
        var h = b.radians(a.startAngle), i = b.radians(a.endAngle), m = i - h;
        if (m < 2 * Math.PI - .000125) {
            var l = h - Math.PI / 2, n = i - Math.PI / 2;
            return new Fb(f, g, e, l, n);
        }
    } for (var c = [], j = this.sharedAxis.actualMaximum, o = 2 * Math.PI / j, f = this.cx, g = this.cy - e, d = 0;
        d < j;
        d++) {
...
M.prototype._initDefs = function () {
    ...
    var b = a.extend(true, {
        ...
        , this.defaults, {
            strokeStyle: "gray", renderLinesOverGraph: true, location: "radial"
        }
    );
    this.defaults = b;
}
...
M.prototype._correctOrigin = function () {
    for (var a = 0, j = this.x, k = this.y, i = this.x + this.width, f = this.y + this.height, g = this._getLabels(), c = 0;
        c < g.length;
        c++) {
        var b = g[c];
...
M.prototype._getLabels = function () {
    ...
    if (g == 0) return;
    for (var i = this.cx, j = this.cy, n = i, o = j - this.radius, m = 2 * Math.PI / g, c = 8, h = [], f = this.labels, e = 0;
        e < g;
        e++) {
        var d = e * m, k = b.rotatePointAt(n, o, d, i, j), l = this.getLabel(e), a = this._createLabel(l, f);
        a.x = k.x;
        a.y = k.y;
        if (d == Math.PI) {
            a.x -= a.width / 2;
...
M.prototype._render = function (n) {
    ...
    if (g == 0) return;
    for (var d = this.cx, e = this.cy, q = d, s = e - this.sharedAxis.extent, r = d, t = e - this.radius, o = this.renderLinesOverGraph, p = 2 * Math.PI / g, i = 0;
        i < g;
        i++) {
        var k = i * p, l = b.rotatePointAt(q, s, k, d, e), m = b.rotatePointAt(r, t, k, d, e), c = new h(l.x, l.y, m.x, m.y);
        c.strokeStyle = this.strokeStyle;
        c.lineWidth = this.lineWidth;
        c.strokeDashArray = this.strokeDashArray;
        if (o) f.push(c);
...
D.prototype._initDefs = function () {
    ...
    var b = a.extend(true, {
        ...
        , this.defaults, {
            minimum: 0, maximum: 360, startAngle: 0, endAngle: 360, renderLinesOverGraph: true, strokeStyle: "gray"
        }
    );
    this.defaults = b;
}
...
D.prototype._getAngle = function (g) {
    var e = this.actualMaximum - this.actualMinimum, f = this.endAngle - this.startAngle, d = b.radians(f), c = b.radians(this.startAngle), a;
    if (e === 0) a = c + d / 2;
...
D.prototype._getLabels = function () {
    ...
    for (var j = this.cx, k = this.cy, p = j + this.radius, q = k, e = 8, g = [], n = this._getMarkInterval(d, true), h = this._getLabelIntervals(n, d), o = this._getIntervalsLength(h), f = 0;
        f < o;
        f++) {
        var i = h[f], m = this.getLabel(i), c = this._getAngle(i), l = b.rotatePointAt(p, q, c, j, k), a = this._createLabel(m, d);
        a.x = l.x;
        a.y = l.y;
        c += Math.PI / 2;
...
D.prototype._render = function (o) {
    var f = [], d = this.cx, e = this.cy, s = d, n = e;
    if (this.sharedAxis) n -= this.sharedAxis.extent;
    for (var t = d, u = e - this.radius, p = this.renderLinesOverGraph, k = this._getIntervals(this.actualInterval), q = this._getIntervalsLength(k), g = 0;
        g < q;
        g++) {
        var r = k[g], j = this._getAngle(r) + Math.PI / 2, l = b.rotatePointAt(s, n, j, d, e), m = b.rotatePointAt(t, u, j, d, e), c = new h(l.x, l.y, m.x, m.y);
        c.strokeStyle = this.strokeStyle;
        c.lineWidth = this.lineWidth;
        c.strokeDashArray = this.strokeDashArray;
        if (p) f.push(c);
    ...
    a.merge(f, i);
    return {
        postShapes: f, contextShapes: i;
    }
};
function lb(b, a) {
    this.series = b;
    this.defaults = {
        calculationType: "standardError", displayType: "both", value: 10, strokeStyle: "black", lineWidth: 2, capLength: 10;
    }
        ;
    this.setOptions(a);
...
lb.prototype._getYValues = function () {
    for (var h = this.series, e = h.arrData, j = e.length, i = h.xAxisType, c, d, g = [], f = 0;
        f < e.length;
        f++) {
        c = e[f];
        if (c === null) continue;
        if (i == "CategoryAxis") if (a.isArray(c) == false) d = c;
        else d = c[1];
        else d = c[1];
        !b.isNull(d) && g.push(d);
    }
...
lb.prototype._calculateErrorAmount = function () {
    ...
    var j = this._getYValues(u), d = j.length;
    switch (this.calculationType) {
        case "standardError": if (d <= 1) return 0;
            for (var o = 0, c = 0;
                c < d;
                c++)o += Math.pow(j[c], 2);
            o = Math.sqrt(o / (d * (d - 1))) / 2;
            return o;
        case "standardDeviation": if (d <= 1) return 0;
            for (var y = b.sum(j), x = y / d, n = 0, c = 0;
                c < d;
                c++)n += Math.pow(j[c] - x, 2);
            n = Math.sqrt(n / (d - 1));
            return n;
        case "percentage": for (var k = 0, m = 0, v = l, w = i, p, h, r, q, c = 0;
            c < d;
            c++) {
            p = j[c];
            ...
            lower: k, upper: m;
                ...
                ;
        case "fixedValue": return this.value;
        case "customValues": for (var k = 0, m = 0, t = l, s = l, e, g, f, c = 0;
            c < d;
            c++) {
...
lb.prototype.getMaxLength = function () {
    var a = this.series, e = a.isVertical, f = 0;
    if (a.realYAxis._getActualVisibleMinimum() === undefined) return 0;
    var d = this.getUpperError(), b = a.realYAxis.getPosition(0), c = a.realYAxis.getPosition(d);
    return isNaN(b) || isNaN(c) ? 0 : Math.abs(b - c) + this.lineWidth / 2 + 2;
}
    ;
lb.prototype._createShapes = function (o) {
    for (var f = this.series, B = f.isVertical, A = f.arrData, D = A.length, C = f.xAxisType, m, p, g, c, q, r, d, s, t, v, w, e, j, k, i = this.displayType, x = this.errorAmount, l = this.capLength / 2, n, y, z, u = 0;
        u < A.length;
        u++) {
        m = A[u];
        if (m === null) continue;
        ...
        if (B) {
            ...
            switch (i) {
                case "lower": e = new h(c, d, q, d);
                    break;
                case "upper": e = new h(r, d, c, d);
                    break;
                default: e = new h(q, d, r, d);
            }
            ...
            switch (i) {
                case "lower": e = new h(c, d, c, s);
                    break;
                case "upper": e = new h(c, t, c, d);
                    break;
                default: e = new h(c, s, c, t);
            }
            e.lineWidth = this.lineWidth;
            e.strokeStyle = this.strokeStyle;
            o.push(e);
...
function bc(a) {
    this.defaults = {
        marker: {
            visible: true, type: "cross", fillStyle: "red", strokeStyle: "red", lineWidth: 4, size: 10;
        }
        , fillStyle: "#cccccc", strokeStyle: "red", lineWidth: 2;
    };
    this.setOptions(a);
}
